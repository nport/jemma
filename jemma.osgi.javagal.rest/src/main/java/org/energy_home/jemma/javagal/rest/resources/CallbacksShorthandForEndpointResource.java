	...
	public void processPost(String body) {
		...
		if (epString == null) {
			...
			try {
				...
				Status _st = new Status();
				_st.setCode((short) GatewayConstants.GENERAL_ERROR);
				_st.setMessage("Error: mandatory " + Resources.PARAMETER_EP + " parameter incorrect (Unsigned32). You provided: "
						+ epString);
				info.setStatus(_st);
				Info.Detail detail = new Info.Detail();
				info.setDetail(detail);
		...
		if (!Util.isUnsigned8(ep)) {
			...
			Status _st = new Status();
			_st.setCode((short) GatewayConstants.GENERAL_ERROR);
			_st.setMessage("Error: mandatory " + Resources.PARAMETER_EP + " parameter's value invalid (Unsigned8). You provided: "
					+ ep);
			info.setStatus(_st);
			Info.Detail detail = new Info.Detail();
			info.setDetail(detail);
		...
		try {
			...
			RestApsMessageListener listener = new RestApsMessageListener(callback, urilistener, rcmal, getRestManager()
