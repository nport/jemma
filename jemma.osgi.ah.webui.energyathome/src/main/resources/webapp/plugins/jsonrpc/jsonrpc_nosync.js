...
var escapeJSONString = (function() {
	// based on Douglas Crockford's Public Domain JavaScript JSON implementation
	// at
	// http://www.json.org/json2.js as of 2008-11-28
	var escapable = /[\\\"\x00-\x1f\x7f-\x9f\u00ad\u0600-\u0604\u070f\u17b4\u17b5\u200c-\u200f\u2028-\u202f\u2060-\u206f\ufeff\ufff0-\uffff]/g, meta = { // table
		// of
		// character
		// substitutions
		'\b' : '\\b',
...
/**
 * Marshall an object to JSON format. Circular references can be handled if the
 * client parameter JSONRpcClient.fixupCircRefs is true. If this parameter is
 * false, an exception will be thrown if a circular reference is detected.
 * 
 * if the client parameter, JSONRpcClient.fixupDuplicates is true then duplicate
 * objects in the object graph are also combined except for Strings
 * 
 ...
 * would be optimized) this would be worth doing on the upload (on the download
 * it's not so important, because gzip handles this) if it's false, then
 * duplicate objects are "re-serialized"
 * 
 * @param o
 *            the object being converted to json
 * 
 * @return an object, { 'json': jsonString, 'fixups': fixupString } or just {
 *         'json' : jsonString } if there were no fixups found.
 */
function toJSON(o) {
	...
	/**
	 * Do the work of converting an individual "sub" object to JSON. Each object
	 * that is processed has a special marker object attached to it, to quickly
	 * detect if it has already been processed and thus handle circular
	 * references and duplicates.
	 * 
	 * @param o
	 *            object being converted into JSON.
	 * 
	 * @param p
	 *            the parent of the object being processed, it should be null or
	 *            undefined if it's the root object.
	 * 
	 * @param ref
	 *            the "reference" of the object in the parent that is being
	...
	function subObjToJSON(o, p, ref) {
		...
		original, parent, circRef, i;

		if (o === null || o === undefined) {
			return "null"; // it's null or undefined, so serialize it as null
		} else if (typeof o === 'string') {
			// todo: handle duplicate strings! but only if they are over a
			...
			// look for an already existing marker which would mean this object
			// has already been processed
			// at least once and therefore either a circular ref or dup has been
			// found!!
			if (o[marker]) {
				...
				while (parent) {
					// if a circular reference was found somewhere along the
					// way,
				...
				// if we found ourselves in the parent chain then this is a
				// circular reference
				if (circRef) {
					// either save off the circular reference or throw an
					// exception, depending on the client setting
					if (JSONRpcClient.fixupCircRefs) {
						// remove last redundant unshifted reference
						fixup.shift();
						original.shift();

						// todo: (LATER) if multiple fixups go to the same
						// original, this could be optimized somewhat
				...
				// mark this object as visited/processed and set up the parent
				// link and prev link
				o[marker] = {
					parent : p,
					prev : markerHead,
				...
				// adjust the "marker" head pointer so the prev pointer on the
				// next object processed can be set
				markerHead = o;
			}

			if (o.constructor === Date) {
				if (o.javaClass) // if Date already has a javaClass defined,
				// use that
				{
				...
				for (i = 0; i < o.length; i++) {
					...
					// if it's a dup/circ ref, put a slot where the object would
					// have been
 ...
 *            this argument is omitted completely, then the JSONRpcClient is
 *            constructed synchronously. if this arguement is an array then it
 *            is the list of methods that can be invoked on the server (and the
 *            server will not be queried for that information)
 * 
 * @param serverURL -
...
JSONRpcClient.prototype.init = function() {
	this._addMethods([ "system.listMethods" ]);
	var req = JSONRpcClient._makeRequest(this, "system.listMethods", [], this.objectID);
	req.deferred = new $.Deferred();
	var promise = req.deferred.promise();
...
function JSONRPCCallableProxy() {
	// A unique identifier which the identity hashcode of the object on the
	// server, if this is a reference type
 ...
 * have the single property "time" (even if no javaClass hint is defined) will
 * also be transformed into JS date objects.
 */
 ...
 * bound to the right object. This is needed as the serverMethodCaller is called
 * only once in createMethod and is then assigned to multiple CallableReferences
 * are created.
 */
...
/*
 * This creates a method that points to the serverMethodCaller and binds it with
 * the correct methodName.
 */
JSONRpcClient._createMethod = function(client, methodName) {
	...
	var serverMethodCaller = function() {
		...
		if (!callback) {
			...
			// if the callback is not specified, we create a deferred and we
			// return it!

			// callback not present, means async request using promises
			req.deferred = new $.Deferred();
...
JSONRpcClient.prototype._addMethods = function(methodNames, dontAdd) {
	...
	for (var i = 0; i < methodNames.length; i++) {
		...
		if ((methodNames[i].substring(0, JSONRpcClient.CALLABLE_REFERENCE_METHOD_PREFIX.length) == JSONRpcClient.CALLABLE_REFERENCE_METHOD_PREFIX)
...
JSONRpcClient._async_handler = function() {
	...
	while (JSONRpcClient.async_responses.length > 0) {
		...
		try {
			if (res.deferred != undefined) {
				res.result = JSONRpcClient.stripJavaClassInfo(res.result);
				res.deferred.resolve(res.result, res.ex, res.profile);
			} else {
...
JSONRpcClient.cancelRequest = function(requestId) {
	...
	/*
	 * If its not in flight yet then we can just mark it as canceled in the the
	 * request queue and it will get discarded before being sent.
	 */
	...
	/*
	 * It may have returned from the network and be waiting for its callback to
	 * be dispatched, so mark it as canceled in the response queue and the
...
JSONRpcClient._sendRequest = function(client, req) {
	...
	/* Send the request */
	http.open("POST", client.serverURL, true, client.user, client.pass);
	...
	if (req.cb || req.deferred != undefined) {
		http.onreadystatechange = function() {
			...
			if (http.readyState == 4) {
				...
				// NICOLA
				if (req.deferred != undefined) {
					res.deferred = req.deferred;
				}
				if (req.profile) {
...
JSONRpcClient.prototype.unmarshallResponse = function(data) {
	 ...
	 * @param fixups
	 *            array of fixups to apply. each element of this array is a 2
	 *            element array, containing the array with the fixup location
	 *            followed by an array with the original location to fix up into
	 *            the fixup location.
	 */
	 ...
	 * with javascript dates. An object is replaced with a JS date when any of
	 * the following conditions is true: The object has a class hint, and the
	 * value of the hint is one of the classes in JSONRpcClient.javaDateClasses
	 * 
	 * The object does not have a class hint, and
	 * JSONRpcClient.transformDateWithoutHint is set to true and the ONE AND
	 * ONLY property is 'time'
	 * 
	 * Note that the traversal creates an infinite loop if the object graph has
	 * any cycles, so do not call this function after fixing up circular refs.
	 * 
	 * @param obj
	 *            root of the object graph where dates should be replaces.
	 * @return object graph where serialized date objects are replaced by JS
	 *         dates.
	 */
	function transformDate(obj) {
		/**
		 * Utility function to determine if an object has one and only
		 * non-function prop.
		 * 
		 * @param obj
		 *            object to check
		 ...
		 * @return true if prop is the ONE and only non-function property in
		 *         obj.
		 */
		...
		if (obj && typeof obj === 'object') {
			if ((obj.javaClass && JSONRpcClient.javaDateClasses[obj.javaClass])) {
				...
				// save the type of java Date transformed directly in the JS
				// date object
				if (obj.javaClass !== 'java.util.Date') {
					// so it can be reassembled if the Date is marshalled back
					// to Java.
 ...
 * recommends
 */
JSONRpcClient.msxmlNames = [ "MSXML2.XMLHTTP.6.0", "MSXML2.XMLHTTP.3.0", "MSXML2.XMLHTTP", "MSXML2.XMLHTTP.5.0",
		"MSXML2.XMLHTTP.4.0", "Microsoft.XMLHTTP" ];
 ...
 */
JSONRpcClient.stripJavaClassInfo = function(json) {
	var i;
	...
	if (json.list != undefined) {
		...
		for (i = 0; i < json.length; i++) {
			var a = JSONRpcClient.stripJavaClassInfo(json[i]);
			json[i] = a;
		}
	} else if (json.map != undefined && $.isPlainObject(json.map)) {
		// maps
		json = json.map;
		json = JSONRpcClient.stripJavaClassInfo(json);
		...
		for ( var property in json) {
			if (json.hasOwnProperty(property)) {
				...
				b = JSONRpcClient.stripJavaClassInfo(json[property]);
				json[property] = b;
			}
		}
		...
		for (i = 0; i < json.length; i++) {
			a = json[i];
			b = JSONRpcClient.stripJavaClassInfo(a);
			json[i] = b;
		}
	}
	return json;
