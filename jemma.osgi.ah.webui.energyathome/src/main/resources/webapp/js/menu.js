 ...
 */
Menu.Init = function(m, mainDiv, contentDiv) {
	...
	if (Main.env == 0)
		console.log(80, "Menu", "Init: main = " + Menu.MainW + " " + Menu.MainH
				+ " dim = " + dim + " dimImage = " + dimImage);
	...
	Menu.MainHtml = "";
	for (i = 0; i < num; i++) {
		Menu.MainHtml = Menu.MainHtml + "<img class='MainElSfondo' src='"
				+ Menu.sfondoElImg + "' style='top:"
				+ topOffset + "px;left:" + leftOffset + "px' + width='" + dim
				+ "px' + height='" + dim + "px'>"
				+ "<div class='MainMenuEl' id='MainEl" + i
				+ "' tabIndex='0' onClick='Menu.OnClickMainMenu(" + i
				+ ")' width='" + dim + "px' height='" + hDiv
				+ "px' style='top:"
				+ (topOffset + extraTopOffset) + "px;left:" + leftOffset
				+ "px' >" + "  <img id='MainImg" + i
				+ "' class='MainMenuImg' width='" + dimImage + "px' height='"
				+ dimImage + "px' src='" + Menu.MainMenu[i].Image
				+ "' style='top:" + imgTopOffset + "px;left:"
				+ imgLeftOffset + "px'>" + "  <p id='NomeMain" + i
				+ "' class='MainMenuTitle'>" + Menu.MainMenu[i].Nome + "</p>"
				+ "</div>";
	...
	if (Main.env == 0)
		console.log('CostiConsumi', CostiConsumi);
	CostiConsumi.Init();
}
...
Menu.OnClickMainMenu = function(val) {
	...
	if (oldContent != undefined) {
		...
		if ((exitFunc != undefined) && (exitFunc != null)) {
			Tracing.Trace(Menu.MainMenu[iMain].SubMenu[iContent].Section,
					Tracing.OUT, null, null);
			eval(exitFunc);
		}
		// tolgo selezione a elemento content menu: cambio icona a immagine,
		// cambio classe al div
		$("#img" + iMain + "Content" + iContent).attr("src",
				Menu.MainMenu[iMain].SubMenu[iContent].Image);
		$("#el" + iMain + "Content" + iContent).removeClass(
	...
	if (mainElId != undefined) {
		...
		if (Main.env == 0)
			console.log(80, "Menu", "OnClickMainMenu : id = " + mainElId
					+ " ind = " + parseInt(mainElId.substr(6)));
	...
	$("#ContentMenu" + val).removeClass("hidden");
	...
	if ((func != undefined) && (func != null)) {
		...
		Tracing.Trace(Menu.MainMenu[val].SubMenu[0].Section, Tracing.IN, null,
				null);
		eval(func);
	} else {
...
Menu.InitContentMenu = function(contentDiv) {
	...
	for (i = 0; i < nMain; i++) {
		...
		Menu.SubMenuHtml = Menu.SubMenuHtml
				+ "<div class='ContentMenuRow' id='ContentMenu" + i + "' >";

		// calcolo distanza tra gli elementi
		for (j = 0; j < num; j++) {
			Menu.SubMenuHtml = Menu.SubMenuHtml
					+ "<div tabindex='0'style='left:"
					+ leftOffset
	...
	// tengo conto della lunghezza dei campi nome per la dimensione del div le
	// icone restano a distanza costante
...
Menu.OnClickContentMenu = function(valMain, valContent) {
	...
	if (Main.env == 0)
		console
				.log(80, "Menu", "OnClickContentMenu oldContent = "
						+ oldContent);
	...
	$("#el" + valMain + "Content" + valContent).addClass(
			"ContentMenuElSelected");
	$("#img" + valMain + "Content" + valContent).attr("src",
			Menu.MainMenu[valMain].SubMenu[valContent].ImageSelected);

	func = Menu.MainMenu[valMain].SubMenu[valContent].FuncEnter;

	if (Main.env == 0)
		console.log(80, "Menu", "valMain = " + valMain);
	if (Main.env == 0)
		console.log(80, "Menu", "valContent = " + valContent);
	if (Main.env == 0)
		console.log(80, "Menu", "Menu = ");
	if (Main.env == 0)
		console.log(80, "Menu", Menu);
	...
	if ((func != undefined) && (func != null)) {
		...
		Tracing.Trace(Menu.MainMenu[valMain].SubMenu[valContent].Section,
				Tracing.IN, null, null);
		eval(func);
	} else {
