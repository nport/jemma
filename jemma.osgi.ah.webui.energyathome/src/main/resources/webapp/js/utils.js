...
/**
 * Gestisce il resize di un'imamgine in modo che sia nel contenitore senza
...
Utils.ResizeImg = function(imgTag, image, set, modo, align) {
	...
	// calcolo offset dall'angolo in alto a sinistra in modo da allineare
	// l'immagine
	// nel contenitore in base alle richieste
	// 1=TopLeft, 2=TopRight, 3=BottomLeft, 4=BottomRight, 5=center height,
	// left, 6=center height, right
...
function sleepJS(mseconds) {
	var start = new Date().getTime();
	for (var i = 0; i < 1e7; i++) {
		if ((new Date().getTime() - start) > mseconds) {
			break;
		}
	}
}
...
ifUtils.getAttribute = function(device, attributeName) {
...
ifUtils.setAttribute = function(device, attributeName, newValue) {
	var measure = device["measure"][attributeName];
	if (measure != null) {
...
ifUtils.getDeviceByPid = function(devices, pid) {
...
ifUtils.isConnected = function(device) {
	return (device.connessione == InterfaceEnergyHome.APP_CONNECTED);
}

ifUtils.appendUnit = function(value, unit) {
	return value + " " + unit;
}

ifUtils.hasCluster = function(device, clusterName) {
	for(ep in device.clusters) {
		var clusters = device.clusters[ep];
		for (i in clusters) {
...
ifUtils.printAttributeValue = function(value) {
	if (value != null) {
		return valeu
	} else {
		return "--";
