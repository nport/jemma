...
function bindService(name) {
	...
	if (InterfaceEnergyHome.jsonrpc == null) {
		try {
			...
			JSONRpcClient.toplevel_ex_handler = function(e) {
				console.log("called jsonrpc exception handler: " + e.message);
			};
		} catch (e) {
			console.log("ERRORE! => ");
			console.log(e);
			InterfaceEnergyHome.GestErrorEH(null, e);
			return null;
		}
	}
	...
	sReg = new Array();
	try {
		var p = InterfaceEnergyHome.jsonrpc.OSGi.find(InterfaceEnergyHome.serviceName);
		p.done(function(sReg) {
			console.log("service found");
			if (sReg.list && sReg.list.length > 0) {
				try {
					var p1 = InterfaceEnergyHome.jsonrpc.OSGi.bind(sReg.list[0]);
					p1.done(function() {
...
InterfaceEnergyHome.BackElettrStorico = function(result, err) {
	if (InterfaceEnergyHome.backElettrStorico != null) {
		...
		// console.log(80, InterfaceEnergyHome.MODULE, "BackElettrStorico val =
		// " + retVal);
...
InterfaceEnergyHome.bindFakeFunctions = function() {
	...
	if (InterfaceEnergyHome.simulateMeasures) {
		...
		service.getAttribute = function(attributeName) {
			...
			if (attributeName == InterfaceEnergyHome.attrs.ATTR_TOTAL_POWER) {
				...
				if (devices.list.length > 0) {
					$.each(devices.list, function(i, device) {
						appType = device["map"][InterfaceEnergyHome.ATTR_APP_TYPE];
						appCategory = device["map"][InterfaceEnergyHome.ATTR_APP_CATEGORY];
						if (appType == InterfaceEnergyHome.SMARTINFO_APP_TYPE) {
							if (appCategory == "12") {
								attributeValue.value = device["map"]["potenza"];
							}
						}
					});
				}
		...
		service.getAttributeData = function() {
			...
			var data = fakeData.Storico.map[fakeElementIndex];
...
InterfaceEnergyHome.parseFakeValues = function(data) {
	...
	$.each(data, function(name, value) {
		if (startsWith(name, 'CustomDevice')) {
			...
			if (fakeData.noServerCustomDevice.list[i] == null) {
				// create the entry
				fakeData.noServerCustomDevice.list[i] = {
					map : {
			...
			if (field == potenza) {
				...
				fakeData.noServerCustomDevice.list[i].map['device_value']
			} else {
				fakeData.noServerCustomDevice.list[i].map[field] = value;
			}
		} else if (name.substring(0) == 'EnergiaProdottaGiornalieroSimul') {
			fakeData.energiaProdotta.list = value.split(" , ").map(function(item) {
				return parseInt(item, 10);
			});
		} else if (name.substring(0) == 'EnergiaConsumataGiornalieroSimul') {
			fakeData.energiaConsumata.list = value.split(" , ").map(function(item) {
				return parseInt(item, 10);
			});
		} else if (name.substring(0) == 'ConsumoMedio') {
			fakeData.ConsumoMedio.list = value.split(" , ").map(function(item) {
				return parseInt(item, 10);
			});
		} else if (name.substring(0) == 'PrevisioneEnergiaProdottaGiornalieroSimul') {
			fakeData.previsioneEnergiaProdotta.list = value.split(" , ").map(function(item) {
				return parseInt(item, 10);
			});
		} else if (name.substring(0) == 'ProdottaMedio') {
			fakeData.prodottaMedio.list = value.split(" , ").map(function(item) {
				return parseInt(item, 10);
			});
		} else if (name.substring(0) == 'ProdottaMedioSettimanale') {
			fakeData.ProdottaMedioSettimanale.list = value.split(" , ").map(function(item) {
				return parseInt(item, 10);
			});
		} else if (name.substring(0) == 'percIAC') {
			fakeData.IAC.list[0] = parseFloat(value, 10);
		} else if (name.substring(0) == 'Forecast') {
			fakeData.Forecast.list[0] = parseInt(value, 10);
		} else if (name.substring(0) == 'MoltForCost') {
			fakeData.MoltForCost.list[0] = parseFloat(value, 10);
		} else if (name.substring(0) == 'SuddivisioneConsumi') {
			fakeData.SuddivisioneConsumi = {
				map : {}
			};

			var map = fakeData.SuddivisioneConsumi.map;
			...
			for (var i = 0; i < value; i++) {
				...
				// fakeData.SuddivisioneConsumi.map[pid].list[i] = value;
			}
		} else if (name.substring(0, 2) == 'SI') {

			fakeData.Storico.map[name] = {
				"list" : new Array()
			};

			fakeData.Storico.map[name].list = data[name].split(' , ').map(function(item) {
				return parseInt(item, 10);
			});
