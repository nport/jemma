var Report = {
	...
	htmlContent : $(document.createElement('div'))
			...
			.attr('class', 'Content')
			.append(
					$(document.createElement('div')).attr('id', 'TitoloReport')
							.attr('class', 'ContentTitle').css('text-align',
									'center').text(Msg.report["report"]))
			.append(
					$(document.createElement('div'))
							...
							.append(
									$(document.createElement('div'))
											...
											.text(
													Msg.report["TitoloEventiCasa"])
											.append(
													$(
															document
																	.createElement('span'))
															.text(
																	Msg.report["carosello"][0])))
							.append(
									$(document.createElement('div')).attr('id',
											'carousell').width('100%').height(
											'55%')))
			.append(
					$(document.createElement('div'))
							...
							.append(
									$(document.createElement('div'))
											.attr('id',
													'TitoloElettrodomDettaglio')
											.attr('class', 'TitoloDettaglio')
											.text(
													Msg.report["TitoloElettrodomDettaglio"]))
							.append(
									$(document.createElement('div'))
											.attr('id', 'ReportInfo')
											.text(
...
Report.Init = function() {
	...
	Highcharts.setOptions({
		colors : [ 'red', "#9EC200", '#ED561B', '#DDDF00', '#24CBE5',
	...
	if (divReport.length == 0) {
		...
		var riga = $(document.createElement('table')).attr('class',
				'accordion-table').height('30%').append(
				$(document.createElement('tbody')).append(
						$(document.createElement('tr')).append(
								...
								$(document.createElement('td')))).append(
						$(document.createElement('tr')).append(
								$(document.createElement('td')).attr('id',
										'posizioneclassifica').attr('class',
										'altro')).append(
								$(document.createElement('td')).attr('id',
										'mediacommunity').attr('class',
										'secondo')).append(
								$(document.createElement('td')).attr('id',
										'previsione').attr('class', 'altro'))));

		/* Aggiungo il Div che conterr� l'accordion */
		$(document.createElement('div')).attr('id', 'EventiCasa1').appendTo(
				$("#ReportDatiCasa"));
																...
																.append(
																		$(
																				document
																						.createElement('span'))
																				.text(
																						ReportSim.consumiFissi))))
								.append(
										$(document.createElement('div')).attr(
												'id', 'Dati11Content').height(
												100))
								.append(
										$(document.createElement('div'))
												.attr('id', 'headerAccordion2')
												.attr('class',
														'headerAccordion')
												.append(
														$(
																...
																.append(
																		$(
																				document
																						.createElement('span'))
																				.text(
																						ReportSim.consumoAnno))))
								.append(
										$(document.createElement('div')).attr(
												'id', 'Dati12Content').height(
												100))
				/*
				 * .append($(document.createElement('div')).attr('id',
				 * 'headerAccordion3').attr('class', 'headerAccordion')
				 * .append($(document.createElement('a')).attr('href', '#')
				 * .text(Msg.report["CO2"] + " : ")
				 * .append($(document.createElement('span')).text(ReportSim.C02))))
				 * .append($(document.createElement('div')).attr('id',
				 * 'Dati13Content').height(100))
				 */
				);
		...
		// $("#ReportInfo").css("border", "#ABD037 solid 2px").css("background",
		// "#fff");
				...
				.accordion(
						{
							...
							changestart : function(event, ui) {
								indiceAccordion = $.inArray(ui.newHeader[0],
										$("#Dati1 .headerAccordion"));
								switch (indiceAccordion) {
								case 0: {
									Report.VisConsumiFissiGraf();
									indiceAccordion = (Report.indiceCarosello != 0) ? indiceAccordion + 1
											: indiceAccordion;
									break;
								}
								case 1: {
								...
								$("#ReportInfo")
										.html(
												Msg.report["informaReport"][indiceAccordion]);
								$("#paragrafoInfo")
										.html(
												Msg.report["informaReport"][indiceAccordion]);
							}
		...
		$("#ReportDatiCasa table")
				.map(
						function(indexTable, domElementSection) {
							$(domElementSection)
									.find("tr")
									.map(
											function(indexRow, domElementRow) {
												$(domElementRow)
														.children("td")
														.map(
																function(
																	...
																	case 0: {
																		...
																		$(
																				domElementCell)
																				.append(
																						Msg.report["TitoliAccordion"][indexTable + 1][indexCell]);
																		break;
																	}
																	case 1: {
																		/*
																		...
																		$(
																				domElementCell)
																				.append(
																						ReportSim.DatiSim[indexTable + 1][indexCell]);
																	}
																	}
																});
									...
									function() {
										...
										if (cloudCarousellObj.frontIndex != Report.indiceCarosello) {
											...
											if (cloudCarousellObj.frontIndex < 0) {
												Report.indiceCarosello = (Report.numEldo)
														+ cloudCarousellObj.frontIndex;
												Report.moveCarousel('left',
														true);
											} else {
												Report.indiceCarosello = cloudCarousellObj.frontIndex;
												Report.moveCarousel('right',
														true);
											}
											// $($("#carousell
											// img")).removeClass("selezionato");
											// $($("#carousell
											// img")[elettrIndex]).addClass("selezionato");
										}
									}, 1000);
						});
		Report.VisConsumiFissiGraf();
...
Report.moveCarousel = function(click, bringToFront) {
	...
	/*
	 * Il primo header cambia nel caso venga selezionato un elettrodomestico
	 * oppure la casa
	 */
	if (Report.indiceCarosello == 0) {
		...
		$("#ReportDatiCasa table tr:nth-child(1)").first().children("td").map(
				function(indexCell, domElementCell) {
					$(domElementCell).html(
							Msg.report["TitoliAccordion"][0][indexCell]);
				});
		$("#ReportDatiCasa table tr:nth-child(2)").first().children("td").map(
				function(indexCell, domElementCell) {
	...
	if (Report.indiceCarosello == tmpCheck) {
		...
		$("#ReportDatiCasa table tr:nth-child(1)").first().children("td").map(
				function(indexCell, domElementCell) {
					$(domElementCell).html(
							Msg.report["TitoliAccordion"][1][indexCell]);
				});
		$("#ReportDatiCasa table tr:nth-child(2)").first().children("td").map(
				function(indexCell, domElementCell) {
	...
	$("#ReportDatiCasa table tr:odd:gt(0)")
			.map(
					function(indexTable, ElementRow) {
						$(ElementRow)
								.children("td")
								.map(
										function(indexCell, domElementCell) {
											$(domElementCell)
													.html(
															ReportSim.DatiSim[indexTable + 2][indexCell]);
										});
					});

	// Incremento o decremento l'indice che mi riporta all'oggetto selezionato
	// dal carosello
	if (!bringToFront) {
		if (click == 'right') {
	...
	$($("#carousell img")[Report.indiceCarosello]).addClass("selezionato");
	$("#TitoloEventiCasa span").html(
			Msg.report["carosello"][Report.indiceCarosello]);
	$("#TitoloElettrodomDettaglio span").html(
			Msg.report["carosello"][Report.indiceCarosello]);
}
...
Report.VisConsumiFissiGraf = function() {
	...
	Highcharts.setOptions({
		lang : {
			...
			// 'Ago', 'Set', 'Ott', 'Nov', 'Dic'],
			shortMonths : [ 'Gennaio', 'Febbraio', 'Marzo', 'Aprile', 'Maggio',
					'Giugno', 'Luglio', 'Agosto', 'Settembre', 'Ottobre',
					'Novembre', 'Dicembre' ],
			weekdays : [ 'Domenica', 'Lunedi\'', 'Martedi\'', 'Mercoledi\'',
	...
	chart = new Highcharts.StockChart(
			{
				...
				series : [ {
					...
					data : (function() {
						...
						var data = [], time = (new Date(2012, 7, 1, 0, 0, 0))
								.getTime(), i;
						for (i = 1; i <= 365 * 2; i++) {
							data.push([ time + i * 1000 * 60 * 60 * 24,
...
Report.VisCompensa = function() {
	...
	$("#elettrodomDetail").empty();
	$("#elettrodomDetail").append(
			$(document.createElement('p')).attr('height', '25%').css('color',
					'green').text('COMPENSA CON')).append(
			$(document.createElement('img')).attr('src',
					'./Resources/Images/albero.png')).append(
			$(document.createElement('img')).attr('src',
					'./Resources/Images/albero.png')).append(
			$(document.createElement('img')).attr('src',
					'./Resources/Images/albero.png')).append(
...
Report.GetElettrodomestici = function() {
	if (Main.env == 0)
		console.log('Report.js', 'GetElettrodomestici', 'Entro!');

	if (InterfaceEnergyHome.mode > 0) {
		try {
			InterfaceEnergyHome.objService
					.getAppliancesConfigurationsDemo(Report.DatiElettrodomesticiCB);
		} catch (err) {
			// if (Main.env == 0) console.log('exception in Report.js - in
			// Report.GetElettrodomestici method: ', err);
	...
	if (Main.env == 0)
		console.log('Report.js', 'GetElettrodomestici', 'Esco!');
}

Report.DatiElettrodomesticiCB = function(result, err) {
	if (Main.env == 0)
		console.log('Report.js', 'DatiElettrodomesticiCB', 'Entro!');
	...
	if (err != null) {
		// if (Main.env == 0) console.log('exception in Report.js - in
		// Report.DatiElettrodomesticiCB method: ', err);
	...
	if (Main.env == 0)
		console.log('Report.js', 'DatiElettrodomesticiCB', 'Esco!');
}

Report.drawEldo = function(listEldo) {
	if (Main.env == 0)
		console.log('Report.js', 'drawEldo', 'Entro!');

	console.log(listEldo);
	for (element in listEldo) {
		if (listEldo[element][InterfaceEnergyHome.ATTR_APP_CATEGORY] == "12") {
			$('#carousell').append(
					$(document.createElement('img')).attr('id', 'imgCrslHome')
							.attr('class', 'cloudcarousel').attr('src',
									'Resources/Images/menu/home_verde.png')
							.attr('alt', 'Casa').attr('title', 'Casa').attr(
									'width', '20%'));
		} else {
			$('#carousell')
					.append(
							$(document.createElement('img'))
									.attr(
											'id',
											'imgCrsl'
													+ listEldo[element][InterfaceEnergyHome.ATTR_APP_NAME])
									.attr('class', 'cloudcarousel')
									.attr(
											'src',
											'Resources/Images/Devices/'
													+ listEldo[element][InterfaceEnergyHome.ATTR_APP_ICON])
									.attr(
											'alt',
											listEldo[element][InterfaceEnergyHome.ATTR_APP_NAME])
									.attr(
	...
	$('#carousell').append(
			$(document.createElement('button')).attr('id', 'left-but').attr(
					'class', 'carouselLeft').css('position', 'absolute').css(
					'top', '5%').css('left', '5%'));
	$('#carousell').append(
			$(document.createElement('button')).attr('id', 'right-but').attr(
					'class', 'carouselRight').css('position', 'absolute').css(
					'top', '5%').css('left', '85%'));

	if (Main.env == 0)
		console.log('Report.js', 'drawEldo', 'Esco!');
}