...
public class M2MNetworkSclManager {
	...
	private static String buildUri(String nwkSclHostAddress, M2MContainerAddress containerId, String startInstanceId,
			String endInstanceId) {
	...
	void setDeviceConfig(M2MDeviceConfigObject deviceConfig) {
		try {
			...
			nwkSclHostAddress = M2MConstants.URL_HTTP_PREFIX + nwkSclBaseUri.getHost() + M2MConstants.URL_PORT_PREFIX
					+ nwkSclBaseUri.getPort();
	...
	public ContentInstanceItemsList getSclContentInstanceItemsList(String user, M2MContainerAddress containerFilterId,
			long instanceId) throws M2MServiceException {
	...
	public ContentInstanceItems getSclContentInstanceItems(String user, M2MContainerAddress containerId, long startInstanceId,
		...
		try {
			...
			M2MUtils.mapDeviceException(LOG, e, "Error while getting content instance items for container " + containerId.getUrl()
