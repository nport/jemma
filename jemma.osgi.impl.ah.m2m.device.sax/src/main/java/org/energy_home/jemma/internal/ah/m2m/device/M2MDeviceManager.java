...
import org.energy_home.jemma.ah.m2m.device.M2MConfigException;
...
public class M2MDeviceManager implements M2MDeviceConfigurator {
	...
	private void connectTask(boolean isStartup) {
		try {
			LOG.debug("M2M Connection setup task: isValid=" + deviceStatus.isValid() + ", isConnected="
	...
	public void addListener(M2MDeviceListener listener) {
		synchronized (this) {
			if (listener == null) {
				throw new RuntimeException("passed listener object is null");
			}
			LOG.info("added listener '" + listener.getClass().getName() + "' to M2MDevice");
	...
	public void removeListener(M2MDeviceListener listener) {
		synchronized (this) {
			...
			if (!listeners.remove(listener)) {
				throw new RuntimeException("removing a not-set listener");
			}
			LOG.info("removed listener '" + listener.getClass().getName() + "' to M2MDevice");
		}
	}
