...
import org.energy_home.jemma.ah.m2m.device.M2MConstants;
...
public class HapServiceManager implements Runnable, M2MDeviceListener {
	...
	private M2MDevice m2mDevice = null;
	...
	public void setM2MDevice(M2MDevice m2mDevice) {
		...
		m2mDevice.addListener(this);
		this.m2mNetworkScl = m2mDevice.getNetworkScl("pippo");
		LOG.debug("M2M Device bound");
	}

	public void unsetM2MDevice(M2MDevice m2mDevice) {
		if (this.m2mDevice == m2mDevice) {
			m2mDevice.removeListener(this);
			this.m2mDevice = null;
		}
	}

	public void setM2MXmlConverter(M2MXmlConverter xmlConverter) {
		this.xmlConverter = xmlConverter;
	}

	public void unsetM2MXmlConverter(M2MXmlConverter xmlConverter) {
		if (this.xmlConverter == xmlConverter) {
			this.xmlConverter = null;
		}
	}

	// FIXME: adds methods addListener(), and removveListener()
	...
	public static boolean checkAttributeIdFilter(String[] attributeIdFilter, String containerNameOrAddressedId) {
		...
		for (int i = 0; i < attributeIdFilter.length; i++) {
			if (attributeIdFilter[i] != null && containerNameOrAddressedId != null
					&& containerNameOrAddressedId.contains(attributeIdFilter[i]))
				return true;
		}
		return false;
	}
					...
					try {
						...
						while (ciQueue.getQueueSize() > 0) {
							if (ciQueue.getLastCreatedContentInstance() == null) {
								...
								if (instance == null)
									LOG.debug("storeQueuedContentInstances: retrieved latest content instance for container"
											+ containerId + ": null\n");
								else {
									// FIXME: NICOLA commented out because now the jaxb classes don't define these methods anymore.
									// LOG.debug("storeQueuedContentInstances: retrieved latest content instance for container"
						...
						LOG.error("storeQueuedContentInstances: error while reading or writing content instances for container "
								+ ciQueue.getContainerId(), e);
					}
				}
			} catch (Exception e) {
	...
	private ContentInstance getContentInstance(Map<String, ContentInstanceItems> currentCisItems, AHContainerAddress containerId)
		...
		if (currentCisItems == null)
			return null;
		ContentInstanceItems ciItems = currentCisItems.get(((AHM2MContainerAddress) containerId).getM2MContainerAdress()
				.getContentInstancesUrl());
		if (ciItems == null)
			return null;
	...
	private ContentInstanceItemsList createContentInstanceItemsList(Map<String, ContentInstanceItems> currentCisItems,
			AHContainerAddress containerIdFilter, String[] containerNamesFilter) {
		return createContentInstanceItemsList(currentCisItems, containerIdFilter, containerNamesFilter,
				M2MNetworkScl.CONTENT_INSTANCE_OLDEST_ID, M2MNetworkScl.CONTENT_INSTANCE_LATEST_ID);
	}

	private ContentInstanceItemsList createContentInstanceItemsList(Map<String, ContentInstanceItems> currentCisItems,
			AHContainerAddress containerIdFilter, String[] containerNamesFilter, long startInstanceId, long endInstanceId) {
		...
		for (Iterator<Map.Entry<String, ContentInstanceItems>> iterator = currentCisItems.entrySet().iterator(); iterator.hasNext();) {
			pairs = iterator.next();
			ciis = (ContentInstanceItems) pairs.getValue();
			if ((containerIdFilter == null || checkItemsOnContainerIdFilter(ciis,
					((AHM2MContainerAddress) containerIdFilter).getM2MContainerAdress()))
	...
	private void initPersistentBuffers(String cid) {
		if (cid == null) {
			...
			batchCisItemsBuffer = new XmlIndexedCircularBuffer(xmlConverter, 0);
			latestCisItemsBuffer = new XmlIndexedCircularBuffer(xmlConverter, 0);
		} else if (!HapServiceConfiguration.USE_PERSISTENT_BUFFER) {
			LOG.debug("Persistent buffers not used");
			batchCisItemsBuffer = new XmlIndexedCircularBuffer(xmlConverter, MAX_BUFFERED_BATCH_REQUESTS);
			latestCisItemsBuffer = new XmlIndexedCircularBuffer(xmlConverter, MAX_BUFFERED_LAST_CIS_ITEMS);
		} else {
			LOG.debug("Persistent buffers used");
			batchCisItemsBuffer = new XmlIndexedCircularBuffer(xmlConverter, MAX_BUFFERED_BATCH_REQUESTS,
					HapServiceConfiguration.BATCH_REQUESTS_DIR_NAME + cid, BATCH_REQUESTS_FILE_NAME_PREFIX);
			LOG.debug("BATCH CIS ITEMS REQUESTS BUFFER LOADED - SIZE:" + batchCisItemsBuffer.getSize());

			latestCisItemsBuffer = new XmlIndexedCircularBuffer(xmlConverter, MAX_BUFFERED_LAST_CIS_ITEMS,
					HapServiceConfiguration.BATCH_REQUESTS_DIR_NAME + cid, CACHE_LATEST_FILE_NAME_PREFIX);
		...
		if (itemsToSend) {
			synchronized (cisItems) {
				...
				currentLatestCisItems = createContentInstanceItemsList(latestCisItems, null,
						HapServiceConfiguration.CACHED_ATTRIBUTE_ID_FILTER);
		...
		try {
			while (currentCisItems != null && batchRequestCounter < SUBSEQUENT_BUFFERED_BATCH_REQUESTS) {
				...
				if (usingBuffer) {
					...
					assert (currentLatestCisItems != null);
					latestCisItemsBuffer.addItem(currentLatestCisItems);
	...
	public void run() {
		...
		if (fastForwardFactor < 0)
			return;
		long batch_request_timeout = (fastForwardFactor == 1) ? HapServiceConfiguration.BATCH_REQUEST_TIMEOUT
				: HapServiceConfiguration.BATCH_REQUEST_TIMEOUT / fastForwardFactor;
		...
		while (serviceStatus.isStarted()) {
			try {
				...
				batch_request_timeout = (fastForwardFactor == 1) ? HapServiceConfiguration.BATCH_REQUEST_TIMEOUT
						: HapServiceConfiguration.BATCH_REQUEST_TIMEOUT / fastForwardFactor;
	...
	public ContentInstanceItemsList getContentInstanceItemsList(String user, AHContainerAddress containerId, long instanceId)
		...
		checkM2MDeviceConnectedStatus();
		try {
			return m2mNetworkScl.getSclContentInstanceItemsList(((AHM2MContainerAddress) containerId).getM2MContainerAdress(),
					instanceId);
		} catch (M2MServiceException e) {
			throw new M2MHapException(HAP_SERVER_COMMUNICATION_PROBLEMS);
		}
	...
	public ContentInstanceItems getContentInstanceItems(String user, AHContainerAddress containerId, long startInstanceId,
		...
		checkM2MDeviceConnectedStatus();
		try {
			System.out.println("AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA TOTOTOTOTOTOTOTOTOT QUI E' NULL!!!!!!!!!");
			return m2mNetworkScl.getSclContentInstanceItems(((AHM2MContainerAddress) containerId).getM2MContainerAdress(),
	...
	public ContentInstanceItemsList getContentInstanceItemsList(String user, AHContainerAddress containerIdFilter,
		...
		checkM2MDeviceConnectedStatus();
		try {
			return m2mNetworkScl.getSclContentInstanceItemsList(
					((AHM2MContainerAddress) containerIdFilter).getM2MContainerAdress(), startInstanceId, endInstanceId);
		} catch (M2MServiceException e) {
			throw new M2MHapException(HAP_SERVER_COMMUNICATION_PROBLEMS);
		}
	}
	...
	private ContentInstance createContentInstance(String user, AHContainerAddress containerId, ContentInstance contentInstance,
			boolean useLocalCache) throws M2MHapException {
		boolean isLocalOnlyM2MDevice = isLocalOnlyM2MDevice();
		if (!isLocalOnlyM2MDevice)
			checkM2MDeviceStartedStatus();
		if (useLocalCache) {
		...
		if (!isLocalOnlyM2MDevice) {
			checkM2MDeviceConnectedStatus();
			try {
				contentInstance = m2mNetworkScl.createSclContentInstance(
						((AHM2MContainerAddress) containerId).getM2MContainerAdress(), contentInstance);
			} catch (M2MServiceException e) {
				throw new M2MHapException(HAP_SERVER_COMMUNICATION_PROBLEMS);
			}
		}
	...
	public ContentInstance createContentInstanceQueued(String user, AHContainerAddress containerId, long instanceId,
			Object content, boolean sync) throws M2MHapException {
		ContentInstance contentInstance = new ContentInstance();
		contentInstance.setContent(content);
		contentInstance.setId(new Long(instanceId));
	...
	public ContentInstance createContentInstanceQueued(String user, AHContainerAddress containerId,
			ContentInstance contentInstance, boolean sync) throws M2MHapException {
		boolean isLocalOnlyM2MDevice = isLocalOnlyM2MDevice();
		if (!isLocalOnlyM2MDevice)
			checkM2MDeviceStartedStatus();
		Map<AHContainerAddress, HapContentInstancesQueue> queuesMap = sync ? syncContentInstancesQueuesMap
		...
		if (containerName != null) {
			...
			if (containerName.equals(AHContainers.attrId_ah_core_config_name)) {
				...
				url = containerId.getUrl();
				url = url.replace(AHContainers.attrId_ah_core_config_category,
						AHContainers.attrId_ah_cluster_ah_ConfigServer_CategoryPid);
				containerId = new AHM2MContainerAddress(url);
			} else if (containerName.equals(AHContainers.attrId_ah_core_config_location)) {
				url = containerId.getUrl();
				url = url.replace(AHContainers.attrId_ah_core_config_location,
						AHContainers.attrId_ah_cluster_ah_ConfigServer_LocationPid);
				containerId = new AHM2MContainerAddress(url);
			}
		}
	...
	public ContentInstance createLocalContentInstance(String user, AHContainerAddress containerId, long instanceId, Object content)
		...
		synchronized (localCisItems) {
			...
			if (contentInstanceAddresssedId.startsWith(M2MConstants.URL_SCL_BASE))
				contentInstanceAddresssedId = contentInstanceAddresssedId.replace(M2MConstants.URL_SCL_BASE,
						M2MConstants.URL_HAG_SCL_BASE);
			ciisLatest.setAddressedId(contentInstanceAddresssedId);
			ciisLatest.getContentInstances().add(contentInstance);
	...
	private void checkDeviceIdUpdate() {
		synchronized (serviceStatus) {
			...
			if (deviceId == null && newDeviceId != null || deviceId != null && newDeviceId == null || deviceId != null
