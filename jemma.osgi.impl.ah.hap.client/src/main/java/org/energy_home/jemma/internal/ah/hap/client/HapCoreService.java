...
public class HapCoreService implements BundleActivator, CommandProvider, M2MHapServiceListener {
	...
	private IM2MHapService m2mHapService = null;
	...
	private void contentInstanceStoreAttributeValue(CommandInterpreter ci, String containerUrl, ContentInstance value,
		...
		if (isBatch)
			result = m2mHapService.createContentInstanceBatch(containerId, value);
		else
			result = m2mHapService.createContentInstance(containerId, value);
		if (result != null) {
	...
	private void contentInstancesCacheRetrieve(CommandInterpreter ci, String containerUrl) {
		...
		try {
			...
			ContentInstanceItemsList itemsList = m2mHapService.getCachedLatestContentItemsList(containerId);
			if (itemsList != null) {
				// FIXME: NICOLA commented out because now the jaxb classes don't define these methods anymore.
	...
	private void contentInstancesRetrieve(CommandInterpreter ci, String containerUrl, long id) {
		...
		try {
			...
			if (!containerId.isFilterAddress()) {
				ContentInstance instance = m2mHapService.getContentInstance(containerId, id);
				if (instance != null) {
					// FIXME: NICOLA commented out because now the jaxb classes don't define these methods anymore.
				...
				ContentInstanceItemsList itemsList = m2mHapService.getContentInstanceItemsList(containerId, id);
				if (itemsList != null) {
					// FIXME: NICOLA commented out because now the jaxb classes don't define these methods anymore.
					// ci.println(itemsList.toXmlFormattedString());
					return;
	...
	private void contentInstancesQuery(CommandInterpreter ci, String containerUrl, long startInstanceId, long endInstanceId,
		...
		try {
			...
			if (!containerId.isFilterAddress()) {
				...
				items = m2mHapService.getContentInstanceItems(containerId, startInstanceId, endInstanceId);
				if (items != null) {
					// FIXME: NICOLA commented out because now the jaxb classes don't define these methods anymore.
				...
				itemsList = m2mHapService.getContentInstanceItemsList(containerId, startInstanceId, endInstanceId);
				if (itemsList != null) {
					// FIXME: NICOLA commented out because now the jaxb classes don't define these methods anymore.
					// ci.println(itemsList.toXmlFormattedString());
					return;
	...
	public synchronized void start(BundleContext bc) throws Exception {
		this.bc = bc;
		// m2mHapService.setUser(null);
		// m2mHapService.setListener(this);
	...
	public void _c(CommandInterpreter ci) {
		boolean connected = m2mHapService.isConnected();
		if (connected == true)
	...
	public void _t(CommandInterpreter ci) {
		long t = m2mHapService.getLastSuccessfulBatchRequestTimestamp();
