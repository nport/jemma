...
public class XmlIndexedCircularBuffer {
	...
	M2MXmlConverter xmlConverter = null;
	...
	private M2MXmlObject loadFromFile(String filePathPrefix, Long fileIndex) {
		String filePath = getFilePath(filePathPrefix, fileIndex);
		return (M2MXmlObject) xmlConverter.loadFromFile(filePath);
	}

	private boolean saveToFile(String filePathPrefix, Long fileIndex, M2MXmlObject object) {
		String filePath = getFilePath(filePathPrefix, fileIndex);
		return xmlConverter.saveToFile(filePath, object);
	}
	...
	public XmlIndexedCircularBuffer(M2MXmlConverter xmlConverter, int maxSize) {
	...
	public XmlIndexedCircularBuffer(M2MXmlConverter xmlConverter, int maxSize, String dirName, String fileNamePrefix) {
		if (maxSize <= 0)
			new IllegalStateException("Size must be a positive integer value");
