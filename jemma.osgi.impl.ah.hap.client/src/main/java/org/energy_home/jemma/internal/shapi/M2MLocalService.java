...
public class M2MLocalService extends HttpServlet implements IApplicationService, ISubscriptionManager, IM2MLocalService {
	...
	private static final String sclsListUri = M2MConstants.URL_HAG_SCL_BASE + M2MConstants.URL_SCLS + M2MConstants.URL_SLASH
			+ M2MContainerAddress.ALL_ID_FILTER;
	...
	class NotificationTask implements Runnable {
		...
		public HttpEntity getEntity(ContentInstanceItems cisItems) {
			...
			try {
				...
				// FIXME: NICOLA: ad a way to print xml
				// LOG.error("Error while creating http entity from " +
				// cisItems.toXmlString(), e);
				return null;
			}
			return entity;
		}
		...
		public void run() {
			synchronized (m2mLocalListenerList) {
				for (Iterator iterator = m2mLocalListenerList.iterator(); iterator.hasNext();) {
					...
					if (containerAddressFilter == null || HapServiceManager.checkItemsOnContainerIdFilter(cisItems,
							containerAddressFilter.getM2MContainerAdress()))
				...
				synchronized (subscriptionInfos) {
					for (Iterator iterator = subscriptionInfos.iterator(); iterator.hasNext();) {
						...
						try {
							...
							if (containerAddressFilter == null || HapServiceManager.checkItemsOnContainerIdFilter(cisItems,
									subscriptionInfo.containerAddressFilter.getM2MContainerAdress())) {
	...
	private M2MConverters m2mConverters;
	...
	private boolean commitApplianceConfigurationUpdates() {
		boolean result = true;
		synchronized (applianceConfigurationUpdatesMap) {
			for (Iterator<IApplianceConfiguration> iterator = applianceConfigurationUpdatesMap.values().iterator(); iterator
					.hasNext();) {
	...
	private void initResources(String sclId) {
		...
		if (sclId == null) {
			...
			// Currently works only with default end point id (1) used in
			// Energy@home trial
	...
	public void setM2MHapService(M2MHapService hapService) {
		...
		try {
			...
			relativeHumidityMeasurementClusterProxy = new RelativeHumidityMeasurementClusterProxy(applianceProxyList,
					this.hapService, this);
			...
			applianceEventsAndAlertsClusterProxy = new ApplianceEventsAndAlertsClusterProxy(applianceProxyList, this.hapService,
					this);
			serviceClusterProxies = new IServiceCluster[] { onOffClusterProxy, meteringClusterProxy,
					temperatureMeasurementClusterProxy, iasClusterProxy, relativeHumidityMeasurementClusterProxy,
					illuminanceMeasurementClusterProxy, applianceControlClusterProxy, applianceEventsAndAlertsClusterProxy };
		} catch (Exception e) {
	...
	public void start() {
		...
		if (xmlConverter == null) {
			throw new RuntimeException("fatal error xmlConverter is null");
		}
		...
		if (requestUri.startsWith(M2MConstants.URL_SCL_BASE)) {
			...
			try {
				...
				if (requestUri.startsWith(subscriptionItemsAddressedId)) {
					...
					// TODO:!!! manage uri with a specific timestamp used for
					// content instance id
					...
					if (containerName != null && containerName.equals(AHContainers.attrId_ah_zigbee_network_status)) {
						...
						// If latest or oldest is specified a ContentInstance is
						// returned, otherwise a ContentInstanceItems
						// with a single item is built
						if (xmlObject != null && contentInstanceId == null) {
							xmlObject = getSingleContentInstanceItems(requestUri, (ContentInstance) xmlObject, startInstanceId,
									endInstanceId);
						}
					} else if (!isFilter) {
						...
						if (clusterAndAttributeNames == null) {
							...
							IServiceCluster serviceCluster = applianceProxy.getServiceCluster(
									new Integer(containerAddress.getEndPointId()).intValue(), clusterAndAttributeNames[0]);
						...
						if (xmlObject != null && contentInstanceId == null) {
							// If latest or oldest is specified a
							// ContentInstance is returned, otherwise a
			...
			try {
				if (requestUri.equals(subscriptionItemsAddressedId)) {
					...
					if (subscription.getId() != null || subscription.getContact() == null) {
						...
						for (Iterator iterator = subscriptionInfos.iterator(); iterator.hasNext();) {
							// Only one subscription for each contact uri is
							// allowed (last subscription overwrite a previous
							// existing one)
		...
		try {
			...
			for (int i = 0; i < endPoints.length; i++) {
				try {
					...
					hapService.storeAttributeValue(appliancePid, new Integer(endPoint.getId()), null, timestamp, endPoint.getType(),
							true);
				} catch (HacException e) {
					LOG.error("applianceAndEndPointTypesUpdated: exception while storing appliance type", e);
				}
	...
	private void applianceStatusUpdated(ApplianceProxy applianceProxy, int appStatus, long timestamp) {
		try {
			...
			String appliancePid = appliance.getPid();
			hapService.storeAttributeValue(appliancePid, IEndPoint.COMMON_END_POINT_ID,
	...
	private void endPointConfigurationUpdated(ApplianceProxy applianceProxy, IEndPoint endPoint, long timestamp) {
		try {
			...
			Integer endPointId = endPoint.getId();
			// hapService.storeAttributeValue(appliancePid, endPointId,
			// ConfigServer.ATTR_NAME_NAME, timestamp,
			// configServer.getName(confirmationRequestContext), true);
			hapService.storeAttributeValue(appliancePid, endPointId, AHContainers.attrId_ah_core_config_name, timestamp,
					configServer.getName(applianceProxy.getApplicationRequestContext()), true);
			// hapService.storeAttributeValue(appliancePid, endPointId,
			// ConfigServer.ATTR_NAME_LOCATION_PID, timestamp, locationPid
			// != null ? new Integer(locationPid) : null, true);
			hapService.storeAttributeValue(appliancePid, endPointId, AHContainers.attrId_ah_core_config_location, timestamp,
					locationPid != null ? new Integer(locationPid) : null, true);
			// hapService.storeAttributeValue(appliancePid, endPointId,
			// ConfigServer.ATTR_NAME_CATEGORY_PID, timestamp, categoryPid
			// != null ? new Integer(categoryPid) : null, true);
			hapService.storeAttributeValue(appliancePid, endPointId, AHContainers.attrId_ah_core_config_category, timestamp,
