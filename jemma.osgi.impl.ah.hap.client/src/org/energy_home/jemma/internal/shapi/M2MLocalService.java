...
public class M2MLocalService extends HttpServlet implements IApplicationService, ISubscriptionManager, IM2MLocalService {
			...
			synchronized (m2mLocalListenerList) {
				for (Iterator iterator = m2mLocalListenerList.iterator(); iterator.hasNext();) {
					...
					if (containerAddressFilter == null
							|| HapServiceManager.checkItemsOnContainerIdFilter(cisItems,
	...
	private ContentInstance postContentInstance(AHContainerAddress containerAddress, ContentInstance ci) throws ApplianceException,
			...
			try {
				...
				if (requestUri.startsWith(subscriptionItemsAddressedId)) {
					if (requestUri.endsWith(M2MContainerAddress.ALL_ID_FILTER)) {
						...
						String subscriptionId = requestUri
								.substring(subscriptionItemsAddressedId.length() + 1, requestUri.length());
			...
			try {
				if (requestUri.equals(subscriptionItemsAddressedId)) {
					...
					if (subscription.getId() != null || subscription.getContact() == null) {
						...
						for (Iterator iterator = subscriptionInfos.iterator(); iterator.hasNext();) {
							// Only one subscription for each contact uri is allowed (last subscription overwrite a previous
							// existing one)
		...
		try {
			...
			for (int i = 0; i < endPoints.length; i++) {
				try {
					...
					hapService.storeAttributeValue(appliancePid, new Integer(endPoint.getId()), null, timestamp,
							endPoint.getType(), true);
				} catch (HacException e) {
					LOG.error("applianceAndEndPointTypesUpdated: exception while storing appliance type", e);
				}
			}
	...
	private void endPointConfigurationUpdated(ApplianceProxy applianceProxy, IEndPoint endPoint, long timestamp) {
		try {
			...
			// hapService.storeAttributeValue(appliancePid, endPointId, ConfigServer.ATTR_NAME_NAME, timestamp,
			...
			// hapService.storeAttributeValue(appliancePid, endPointId, ConfigServer.ATTR_NAME_LOCATION_PID, timestamp, locationPid
			...
			// hapService.storeAttributeValue(appliancePid, endPointId, ConfigServer.ATTR_NAME_CATEGORY_PID, timestamp, categoryPid
