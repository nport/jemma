...
public abstract class AppliancesBasicProxy extends Appliance implements IAppliancesBasicProxy, IServiceClustersListener {
								...
								if (subscriptions != null) {
									for (Iterator iterator = subscriptions.entrySet().iterator(); iterator.hasNext();) {
										...
										try {
											...
											lastNotifiedTimestamp = (attributeValue != null) ? Math
													.max(attributeValue.getTimestamp(),
															applianceStatus.getLastSubscriptionRequestTime()) : applianceStatus
													.getLastSubscriptionRequestTime();
											if (subscriptionParameters != null
												...
												if (System.currentTimeMillis() - lastNotifiedTimestamp > SUBSCRIPTION_MAX_DELAY_FACTOR
													...
													System.out.println("Subscription reneweing for attribute " + attributeName
															+ ", cluster " + serviceCluster.getName() + "," + " ep "
															+ serviceCluster.getEndPoint().getId() + ", appliance "
															+ appliancePids[i] + " -- parameters- MaxReportInterval: "
															+ subscriptionParameters.getMaxReportingInterval()
															+ " -- MinReportInterval: "
															+ subscriptionParameters.getMinReportingInterval()
															+ " -- ReportChange: " + subscriptionParameters.getReportableChange());
													returnedSubscriptionParameters = serviceCluster.setAttributeSubscription(
															attributeName, subscriptionParameters, confirmedRequestContext);
													...
													if (returnedSubscriptionParameters != null
															|| !isFasterSubscriptionCheckEnabled())
											...
											LOG.warn("Error while subscribing attribute with invalid reporting time - "
													+ attributeName, e);
										}
									}
								}
	...
	public void start(ComponentContext ctxt) {
		...
		if (isHacDriverModeActive()) {
			...
			hacServiceRegistration = bc.registerService(
					new String[] { IHacService.class.getName(), CommandProvider.class.getName() }, new SimpleHacService(
							(AppliancesProxy) this), props);
			hacDriverLocatorRegistration = bc.registerService(new String[] { DriverLocator.class.getName() },
					new SimpleHacDriverLocator(), null);
		}
	}
					...
					try {
						...
						synchronized (mainEndPoint) {
							...
							if (serviceClusterProxy == null) {
								...
								serviceClusterProxy = new ServiceClusterProxy(this, clusterInterfaceClass,
										new IEndPointRequestContextCheck() {
											public void checkRequestContext(IEndPointRequestContext context)
	...
	public void notifyAttributeValue(String clusterName, String attributeName, IAttributeValue attributeValue,
		...
		if (applianceStatus == null || applianceStatus.getStatus() != ManagedApplianceStatus.STATUS_ENABLED) {
			LOG.warn("notifyAttributeValue received from " + ((applianceStatus == null) ? "an unknown" : "a not enabled ")
					+ " appliance: " + endPointRequestContext.getPeerEndPoint().getAppliance().getPid() + " " + clusterName + " "
		...
		LOG.debug("notifyAttributeValue: " + endPointRequestContext.getPeerEndPoint().getAppliance().getPid() + " " + endPointId
				+ " " + clusterName + " " + attributeName + " " + attributeValue.getTimestamp() + " " + attributeValue.getValue());
		synchronized (attributeValuesListenerList) {
	...
	public void notifyReadResponse(String clusterName, String attributeName, IAttributeValue attributeValue,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(endPointRequestContext.getPeerEndPoint().getAppliance().getPid() + " - notifyReadResponse " + clusterName
					+ " " + attributeName + " " + attributeValue.getTimestamp() + " " + attributeValue.getValue());
		}
	}
	...
	public void notifyWriteResponse(String clusterName, String attributeName, IAttributeValue attributeValue,
			IEndPointRequestContext endPointRequestContext) throws ServiceClusterException, ApplianceException {
		if (LOG.isDebugEnabled()) {
			LOG.debug(endPointRequestContext.getPeerEndPoint().getAppliance().getPid() + " - notifyWriteResponse " + clusterName
					+ " " + attributeName + " " + attributeValue.getTimestamp() + " " + attributeValue.getValue());
		}
	}
	...
	public void notifyCommandResponse(String clusterName, String commandName, Object response,
		...
		if (LOG.isDebugEnabled()) {
			LOG.debug(endPointRequestContext.getPeerEndPoint().getAppliance().getPid() + " - notifyCommandResponse " + clusterName
