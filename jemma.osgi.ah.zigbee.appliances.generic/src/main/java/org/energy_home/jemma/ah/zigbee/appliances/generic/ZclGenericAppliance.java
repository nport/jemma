...
public class ZclGenericAppliance extends ZclAppliance {
	...
	static class ZclEndPointDescriptor {
		...
		public ZclEndPointDescriptor(int appEndPointId, int profileId, int deviceId, int endPointId, int[] clientClusterIds,
				int[] serverClusterIds) {
	...
	private ZclEndPoint zclAllocateEndPoint(int app_end_point_id, int profile_id, int device_id, int end_point_id,
			int manufacturer_code, int[] clientClusterIds, int[] serverClusterIds) throws ApplianceException {
	...
	public ZclGenericAppliance(String pid, Dictionary config) throws ApplianceException {
		...
		// try {
		// int[] endPointIdsPrimitive = (int[])
		// config.get(IAppliance.APPLIANCE_EPS_IDS_PROPERTY);
		...
		// }
		// } catch (Exception e) {
		// // FIXME: Sometimes an array of Integer object is stored in
		// configuration admin
		// endPointIds = (Integer[])
		// config.get(IAppliance.APPLIANCE_EPS_IDS_PROPERTY);
		// }
		// String[] endPointTypes = (String[])
		// config.get(IAppliance.APPLIANCE_EPS_TYPES_PROPERTY);
		...
		for (Iterator iterator = customConfiguration.iterator(); iterator.hasNext();) {
			...
			zclAllocateEndPoint(epd.appEndPointId, epd.profileId, epd.deviceId, epd.endPointId, 0, epd.clientClusterIds,
					epd.serverClusterIds);
		}
		return;
	}
	...
	protected void attached() {
		LOG.debug("attached");
		// XXX Here can be added with a timer critical initialization code (e.g.
		// sleeping end device reporting configuration,
		// reading zone status type,...)
		// Also additional custom appliance configuration can be updated (e.g.
		// device subtype like ias contact, ias leak, ...)
	}
	...
	protected synchronized ZclEndPoint zclGetEndPoint(int epsNumber, int profile_id, int device_id, int end_point_id,
			int manufacturer_code, List clientClusterIds, List serverClusterIds) throws ApplianceException {
		ZclEndPoint endPoint = null;
		int applEndPointId = end_point_id;
		if (epsNumber == 1)
			// Used for compatibility: device with only one end point always use
			// end point id 1
		...
		if (endPoint == null) {
			...
			endPoint = zclAllocateEndPoint(applEndPointId, profile_id, device_id, end_point_id, manufacturer_code,
					clientIntClusterIds, serverIntClusterIds);
			ZclEndPointDescriptor epd = new ZclEndPointDescriptor(applEndPointId, profile_id, device_id, end_point_id,
					clientIntClusterIds, serverIntClusterIds);
			customConfiguration.add(epd);
		}
		return endPoint;
