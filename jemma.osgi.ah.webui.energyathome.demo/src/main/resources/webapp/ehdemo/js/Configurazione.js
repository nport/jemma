...
Configurazione.HandleServiceEvent = function(status) {
	if (status == InterfaceEnergyHome2.CONNECTED) {
		hideSpinner();
		Configurazione.StartPolling();
	} else if (status == InterfaceEnergyHome2.DISCONNECTED) {
		showSpinner();
...
Configurazione.getPowerLimitCb = function(attributeValue, e) {
	...
	InterfaceEnergyHome2.getHapConnectionId(Configurazione.IdUtenteCb);
}
...
Configurazione.setAttributeCbPV = function(result, e) {
	if (e != null) {
		//KO!
		InterfaceEnergyHome2.setAttribute(Configurazione.setAttributeCb, "PeakProducedPower", 0);
	} else {
		//ok registro i cookie
		InterfaceEnergyHome2.setAttribute(Configurazione.setAttributeCb, "PeakProducedPower", Configurazione.limitProduction);
	}
}
...
Configurazione.setAttributeCb = function(result, e) {
	...
	InterfaceEnergyHome2.setHapConnectionId(Configurazione.setHapConnectionIdCb, Configurazione.idUtente);
}
...
Configurazione.SalvaUtente = function() {
	...
	InterfaceEnergyHome2.setAttribute(Configurazione.setAttributeCbPV, "InstantaneousPowerLimit", Configurazione.limit);
}
...
Configurazione.GestUtente = function() {
	...
	InterfaceEnergyHome2.getAttribute(Configurazione.GestConfigurazione, "PeakProducedPower");
}
...
Configurazione.GestConfigurazione = function(result, e) {
	...
	InterfaceEnergyHome2.getAttribute(Configurazione.getPowerLimitCb, "InstantaneousPowerLimit");
}

Configurazione.ExitElettrodomestici = function() {
	InterfaceEnergyHome2.removeListener("service.connection", Configurazione.HandleServiceEvent);
	Configurazione.cancelTimeouts();
...
Configurazione.DatiCategorie = function(lista) {
	if (lista != null) {
		 ...
		 * 		Configurazione.optionsCategorie += "<option value='" + Configurazione.categorie[i][InterfaceEnergyHome2.ATTR_CATEGORY_PID] + "' class='OptionConf'>" + Configurazione.categorie[i][InterfaceEnergyHome2.ATTR_CATEGORY_NAME] + "</option>";
		 */
	}
...
Configurazione.GetLocazioniCb = function(lista) {
	...
	if (lista != null) {
		 ...
		 * 		Configurazione.optionsLocazioni += "<option value='" + Configurazione.locazioni[i][InterfaceEnergyHome2.ATTR_LOCATION_PID] + "' class='OptionConf'>" + Configurazione.locazioni[i][InterfaceEnergyHome2.ATTR_LOCATION_NAME] + "</option>";
		 */
	}
...
Configurazione.InserisciDispositivo = function() {
	...
	Configurazione.nuovoDispositivo.map[InterfaceEnergyHome2.ATTR_APP_NAME] = $("#NomeElettr").val();
	catPid = $("#CategoriaElettr option:selected").val();
	Configurazione.nuovoDispositivo.map[InterfaceEnergyHome2.ATTR_APP_CATEGORY] = catPid;
	...
	Configurazione.nuovoDispositivo.map[InterfaceEnergyHome2.ATTR_APP_LOCATION] = locPid;
	var tmp = $("#IconaElettr").attr("src");
	var j = tmp.lastIndexOf("/");
	Configurazione.nuovoDispositivo.map[InterfaceEnergyHome2.ATTR_APP_ICON] = tmp.substr(j + 1);

	InterfaceEnergyHome2.InstallAppliance(Configurazione.InseritoDispositivoCb, Configurazione.nuovoDispositivo);
...
Configurazione.ModificaDispositivo = function() {
	...
	Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_NAME] = $("#NomeElettr").val();
	catPid = $("#CategoriaElettr option:selected").val();
	Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_CATEGORY] = catPid;
	locPid = $("#LocazioneElettr option:selected").val();
	//console.log(80, Configurazione.MODULE, "InserisciDispositivo: loc = " + locPid + " cat = " + catPid);
	Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_LOCATION] = locPid;
	var tmp = $("#IconaElettr").attr("src");
	var j = tmp.lastIndexOf("/");
	Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_ICON] = tmp.substr(j + 1);
	...
	newConfig.map[InterfaceEnergyHome2.ATTR_APP_NAME] = Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_NAME];
	newConfig.map[InterfaceEnergyHome2.ATTR_APP_CATEGORY] = Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_CATEGORY];
	newConfig.map[InterfaceEnergyHome2.ATTR_APP_LOCATION] = Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_LOCATION];
	newConfig.map[InterfaceEnergyHome2.ATTR_APP_ICON] = Configurazione.infoDisp[Configurazione.indSel].map[InterfaceEnergyHome2.ATTR_APP_ICON];
	...
	InterfaceEnergyHome2.ModificaDispositivo(Configurazione.ModificaDispositivoCb, newConfig);
}

Configurazione.EliminatoDispositivo = function(e) {
	//console.log(80, Configurazione.MODULE, "EliminatoDispositivo");
	InterfaceEnergyHome2.GetListaAppliances(Configurazione.DatiElettrodomesticiCb);
}
	...
	$.dialog(Msg.config["confermaCanc"],{
						...
						'exit' : function(value) {
							if (value == Msg.config["OK"]) {
								InterfaceEnergyHome2.EliminaDispositivo( Configurazione.EliminatoDispositivo,
																		Configurazione.infoDisp[ind].map[InterfaceEnergyHome2.ATTR_APP_PID]);
...
Configurazione.ConfiguraElettr = function(elem) {
	...
	$("#NomeElettr").val(elem.map[InterfaceEnergyHome2.ATTR_APP_NAME]);
	$("#CategoriaElettr").html(Configurazione.optionsCategorie);
	catPid = elem.map[InterfaceEnergyHome2.ATTR_APP_CATEGORY];
	...
	locPid = elem.map[InterfaceEnergyHome2.ATTR_APP_LOCATION];
	...
	tmpIcon = elem.map[InterfaceEnergyHome2.ATTR_APP_ICON];
	...
	$("#TextTipoElettr").text(elem.map[InterfaceEnergyHome2.ATTR_APP_TYPE]);
	$("#TextIdElettr").text(elem.map[InterfaceEnergyHome2.ATTR_APP_PID]);
	...
	$.dialog(Msg.config["trovatoPlug"],{
						...
						'exit' : function(value) {
							if (value == Msg.config["buttonConfigura"]) {
								InterfaceEnergyHome2.GetCategorie(Configurazione.DatiCategorie,
																 Configurazione.nuovoDispositivo.map[InterfaceEnergyHome2.ATTR_APP_PID]);
							}
						}}
	);
}
	...
	if (e == null) {
		...
		$.dialog(Msg.config["insPlug"], {
			...
			'exit' : function(value) {
				...
				InterfaceEnergyHome2.stopInquiry(Configurazione.StopInquiryCb);
			}
		});
	} else {
		//console.log(80, Configurazione.MODULE, "Errore in StartInquiry");
...
Configurazione.AggiungiElettrodomestico = function() {
	...
	showSpinner();
	InterfaceEnergyHome2.startInquiry(Configurazione.StartInquiryCb, Configurazione.TIMEOUT_INQUIRY);
}
...
Configurazione.VisElettrodomestici = function() {
	...
	for (i = 0; i < Configurazione.numDisp; i++) {
		...
		imgDisp = Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_ICON];
		if (typeof(imgDisp) === 'undefined'){
			imgDisp = Configurazione.icone[5];
		}
			
		availability = Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_AVAIL];
		device_value = Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_VALUE];
		...
		if ((availability == undefined) || (availability != 2)) {
			...
			// if (Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_STATE] == 1) {
			// 		htmlStato = "<div class='StatoOn'>" + Msg.config["statoAcceso"] + "</div>";
			// } else if (Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_STATE] == 0) {
		...
		htmlElettr += "<div id='Elettr_" + i + "' class='ElettrVis'>"
					...
					+ 			Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_NAME]
					+ "		</div>"
					+ "		<div class='ValueElettr' id='ValueElettr_" + i + "'>" + val + "</div>"
					+ 		htmlStato
					// + "<div class='IdElettr' id='IdElettr_" + i + "'>"
					// + Configurazione.infoDisp[i].map[InterfaceEnergyHome2.ATTR_APP_PID]
					// + "</div>"
		...
		$("#ButtonModificaElettr_" + i).click(function() {
							...
							InterfaceEnergyHome2.GetCategorie(Configurazione.DatiCategorie,
															 Configurazione.infoDisp[Configurazione.indSel]
																		   .map[InterfaceEnergyHome2.ATTR_APP_PID]);
		});
...
Configurazione.getHapLastUploadTimeCb = function(timestamp, err) {
	...
	Configurazione.refreshHapStatus();
	if (Configurazione.timerStato == null)
		Configurazione.timerStato = setInterval("InterfaceEnergyHome2.GetStatoConnessione(Configurazione.StatoConnessioneCb)",Configurazione.TIMOUT_STATO);
}
...
Configurazione.StatoConnessioneCb = function(status) {
	...
	InterfaceEnergyHome2.getHapLastUploadTime(Configurazione.getHapLastUploadTimeCb);
}

Configurazione.StartPolling = function() {

	// legge le locazioni e successivamente l'elenco dispositivi
	InterfaceEnergyHome2.GetLocazioni(Configurazione.GetLocazioniCb);
	// legge lo stato della connessione con la piattaforma
	InterfaceEnergyHome2.GetStatoConnessione(Configurazione.StatoConnessioneCb)
}
...
Configurazione.GestElettrodomestici = function() {
	...
	InterfaceEnergyHome2.addListener("service.connection", Configurazione.HandleServiceEvent);
