...
public class PowerProfileInfo {
	...
	public static final int TIME_TOLERANCE_EQUALITY = 5 * 60 * 1000; // 5
																		// minutes
	public static final int INDEFINITE_DURATION = 0xffff;

	public static class PowerProfileState {
		/*
		 * The PowerProfileState allows a device server to communicate its
		 * current Power Profile(s) to the client. The Power Profile record
		 * support the following fields: - Power Profile ID: the identifier of
		 * the Power Profile as requested; - Energy Phase ID: The current Energy
		 * Phase ID of the specific Profile ID; this value shall be set to
		 * invalid 0xFF when PowerProfileState indicates a Power Profile in
		 * POWER_PROFILE_IDLE state - PowerProfileRemoteControl: it indicates if
		 * the PowerProfile is currently remotely controllable or not; if the
		 * Power Profile is not remotely controllable is cannot be scheduled by
		 * a Power Profile client - PowerProfileState: an enumeration field
		 * representing the current state of the Power Profile
		 * 
		 ...
		 * ENERGY_PHASE_WAITING_TO_START 0x05 The Power Profile is in between
		 * two energy phases (one ended, the other not yet started). If the
		 * first EnergyPhase is considered, this state indicates that the whole
		 * power profile is not yet started, but it has been already programmed
		 * to start ENERGY_PHASE_WAITING_PAUSED 0x06 The Power Profile is set to
		 * Pause when being in the ENERGY_PHASE_WAITING_TO_START state.
		...
		public static final String getNameOf(PowerProfileState pps) {
			...
			case POWER_PROFILE_IDLE:
				return "POWER_PROFILE_IDLE";
			case POWER_PROFILE_PROGRAMMED:
				return "POWER_PROFILE_PROGRAMMED";
			case ENERGY_PHASE_RUNNING:
				return "ENERGY_PHASE_RUNNING";
			case ENERGY_PHASE_PAUSED:
				return "ENERGY_PHASE_PAUSED";
			case ENERGY_PHASE_WAITING_TO_START:
				return "ENERGY_PHASE_WAITING_TO_START";
			case ENERGY_PHASE_WAITING_PAUSED:
				return "ENERGY_PHASE_WAITING_PAUSED";
			case POWER_PROFILE_ENDED:
				return "POWER_PROFILE_ENDED";
			default:
				return "UNKNOWN_POWER_PROFILE_STATE " + pps.state;
			}
		}
		...
		public boolean isApplianceStarted() {
			...
			if (energyPhaseId > 1)
				return true;
			if (state == ENERGY_PHASE_PAUSED || state == ENERGY_PHASE_RUNNING)
				return true;
	...
	public static class PowerProfileTimeConstraints {
		...
		public PowerProfileTimeConstraints(short id, int start, int end) {
			...
			if (start < 0)
				start = 0;
			if (end == INDEFINITE_DURATION || end < 0)
				end = MAX_SCHEDULING_INTERVAL;
			startAfter = start;
			stopBefore = end;
			isValid = start < end && start <= MAX_SCHEDULING_DELAY && end - start <= MAX_SCHEDULING_INTERVAL;
		}
	...
	public EnergyPhaseScheduleTime[] getProposedScheduledEnergyPhases() {
		...
		for (int i = 1; i < energyPhases.length; ++i) {
			proposedSchedule[i] = new EnergyPhaseScheduleTime(energyPhases[i].getEnergyPhaseID(),
					energyPhases[i].getScheduledSlot());
		}
	...
	public long getApplianceScheduleTime() {
		if (applianceScheduledPhases.length == 0)
			return 0;
		return applianceScheduleTimeNotification + applianceScheduledPhases[0].getScheduledDelay() * MILLISECS_IN_ONE_MINUTE;
	}
