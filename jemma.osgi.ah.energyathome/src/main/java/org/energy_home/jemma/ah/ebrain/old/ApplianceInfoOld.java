 ...
 */
@Deprecated
// FIXME by Riccardo seems like an old version: can we remove it ?
public class ApplianceInfoOld {
	...
	public static final short TRAILING_DIGIT_CENTS = 2; // trailing digits:
														// cents of euros
	public static final short TRAILING_DIGIT_TENTHS = 3; // trailing digits:
															// thousandths of
															// euros
	public static final int MILLISECONDS_IN_MINUTE = 60 * 1000;
	public static long VALID_TIME_NOTIFICATION_INTERVAL = 12 * 60 * 60 * 1000; // 12
																				// hours
	...
	private void allocateProfileInfo(short totalNum) {
		if (powerProfileInfos == null)
			powerProfileInfos = new PowerProfileInfo[totalNum + 1];
	...
	private PowerProfileInfo getPowerProfileInfo(short profileId) {
		if (powerProfileInfos == null || profileId >= powerProfileInfos.length)
			allocateProfileInfo(profileId);
	...
	/*
	 * The TotalProfileNum attribute represents the total number of profiles
	 * supported by the device.
	 */
	private short totalProfileNum;
	/*
	 * The MultipleScheduling attribute specifies if the server side of the
	 * Power Profile cluster supports the scheduling of multiple Energy Phases
	 * or it does support the scheduling of a single energy phase of the Power
	 * profile at a time.
	 */
	private boolean multipleSchedulingSupported;
	/*
	 * The EnergyFormatting attribute SummationFormatting provides a method to
	 * properly decipher the number of digits and the decimal location of the
	 * values found in the Summation Information Set of attributes. This
	 * attribute is to be decoded as follows: Bits 0 to 2: Number of Digits to
	 * the right of the Decimal Point. Bits 3 to 6: Number of Digits to the left
	 * of the Decimal Point. Bit 7: If set, suppress leading zeros.
	 */
	private short energyFormatting;
	/*
	 * The EnergyRemote attribute describes the remote control status of the
	 * power profile server cluster (e.g. appliance), which means the selection
	 * operated by the user on the remote control feature of theDevice.
	 */
	// private boolean energyRemote;
	/*
	 * The ScheduleMode attribute describes the criteria that should be used by
	 * the Power Profile cluster client side (e.g. energy management system) to
	 * schedule the power profiles.
	 */
	// private short scheduleMode;

	/*
	 * The PowerProfileState allows a device server to communicate its current
	 * Power Profile(s) to the client. The Power Profile record support the
	 * following fields: - Power Profile ID: the identifier of the Power Profile
	 * as requested; - Energy Phase ID: The current Energy Phase ID of the
	 * specific Profile ID; - PowerProfileRemoteControl: it indicates if the
	 * PowerProfile is currently remotely controllable or not; if the Power
	 * Profile is not remotely controllable is cannot be scheduled by a Power
	 * Profile client - PowerProfileState: an enumeration field representing the
	 * current state of the Power Profile
	 * 
	 * Enumeration Value Description POWER_PROFILE_IDLE 0x00 The PP is not
	 * defined in its parameters. POWER_PROFILE_PROGRAMMED 0x01 The PP is
	 * defined in its parameters but without a scheduled time reference
	 * ENERGY_PHASE_RUNNING 0x03 An energy phase is running ENERGY_PHASE_PAUSED
	 * 0x04 The current energy phase is paused ENERGY_PHASE_WAITING_TO_START
	 * 0x05 The Power Profile is in between two energy phases (one ended, the
	 * other not yet started). If the first EnergyPhase is considered, this
	 * state indicates that the whole power profile is not yet started, but it
	 * has been already programmed to start ENERGY_PHASE_WAITING_PAUSED 0x06 The
	 * Power Profile is set to Pause when being in the
	 * ENERGY_PHASE_WAITING_TO_START state. POWER_PROFILE_ENDED 0x07 The whole
	 * Power profile is terminated
	 */
	...
	private ApplianceInfoOld(String id) {
		applianceId = id;
		// retrieve and set all/some attributes of the remote appliance (shown
		// below).
	}
	...
	public PowerProfile getPowerProfileState(short profileId) {
		...
		if (now - ppi.currentScheduleTimeNotification > VALID_TIME_NOTIFICATION_INTERVAL)
			return null;
	...
	public boolean setEffectiveScheduledPhases(short profileId, ScheduledPhase[] scheduledPhases) {
		...
		/*
		 * // check if accepted if (ppi.proposedSchedule != null) { for (int i =
		 * ppi.currentSchedule.length; --i >= 0;) { long t1 =
		 * ppi.currentSchedule[i].ScheduledTime +
		 * ppi.currentScheduleTimeNotification / MILLISECONDS_IN_MINUTE; long t2
		 * = ppi.proposedSchedule[i].ScheduledTime +
		 * ppi.proposedScheduleTimeNotification / MILLISECONDS_IN_MINUTE; //
		 * give one minute tolerance if (Math.abs(t1 - t2) > 1) return false; }
		 * }
		 */
		return true;
	}
	...
	public long getStartConstraint(short profileId) {
		PowerProfileInfo ppi = getPowerProfileInfo(profileId);
		long now = System.currentTimeMillis();
		if (now - ppi.contraintTimeNotification > VALID_TIME_NOTIFICATION_INTERVAL)
			return 0;
	...
	public float getEnergyDecimalConverter() {
		...
		if (decimals > 0)
			return (float) (1 / Math.pow(10, decimals));
		return 1;
	}
