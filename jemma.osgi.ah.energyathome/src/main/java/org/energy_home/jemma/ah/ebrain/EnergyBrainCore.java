...
public class EnergyBrainCore extends MeteringCore implements IPowerAndControlListener {
	...
	public static final int TIME_TOLERANCE_EQUALITY = 3 * 60 * 1000; // 3
																		// minutes
	...
	public void notifyPowerProfile(String applianceId, PowerProfileInfo ppi) {
		...
		if (formatting <= 0) {
			formatting = 1;
			// formatting =
			// powerControlProxy.getEnergyFormatting(appliance.getApplianceId());
	...
	public void notifyAllPowerProfilesState(String applianceId, PowerProfileState[] powerProfilesState) {
		...
		for (int i = 0; i < powerProfilesState.length; ++i) {
			...
			if (ppi != null)
				ppi.setProfileCurrentState(powerProfilesState[i]);
		}
	}
	...
	public void notifyEnergyPhasesScheduleTime(String applianceId, short powerProfileID, EnergyPhaseScheduleTime[] scheduledPhases) {
		...
		if (ppi != null)
			ppi.setApplianceScheduledEnergyPhases(scheduledPhases);
	}
	...
	public void notifyPowerProfileScheduleConstraints(String applianceId, PowerProfileTimeConstraints profileConstraints) {
		if (!profileConstraints.isValid())
			throw new IllegalArgumentException(String.format("Invalid Time Constraints, start-after=%d, stop-before=%d",
					profileConstraints.getStartAfterDelay(), profileConstraints.getStopBeforeDelay()));

		WhiteGoodInfo appliance = getWhiteGoodInfo(applianceId);
		PowerProfileInfo ppi = getOrRetrievePowerProfile(appliance, profileConstraints.getPowerProfileID());
		if (ppi != null)
			ppi.setTimeConstraints(profileConstraints);
	}
	...
	public float calculatePowerProfilePrice(String applianceId, short powerProfileID, int delay) {
		...
		if (ppi.getProfileCurrentState().isApplianceStarted())
			throw new IllegalStateException("Cannot Calculate Price of a Running Appliance - "
					+ PowerProfileState.getNameOf(ppi.getProfileCurrentState()));

		Calendar c = Calendar.getInstance();
		c.add(Calendar.MINUTE, delay);
		long priceRequestTime = c.getTimeInMillis();

		// check to see if this calculation refers to the latest scheduling
		// notification
		...
		for (int i = 0; i < phases.length; ++i) {
			if (phases[i].getPeakPower() > maxPeakPower)
				maxPeakPower = phases[i].getPeakPower();

			float oneSlotEnergy = phases[i].getOneSlotMeanEnergy();
			int duration = phases[i].getSlotDuration();

			// crucial piece of code: here is where the scheduling of each phase
			// is considered
		...
		if (maxPeakPower >= available)
			powerControlProxy.notifyOverloadWarning(applianceId,
					IPowerAndControlProxy.OVERALL_POWER_POTENTIALLY_ABOVE_AVAILABLE_POWER_LEVEL_ON_START);

		return cost;
	}

	private float getCurrentAvailablePower() {
		// float available = upperPowerThreshold -
		// super.getTotalInstantaneosPowerInfo(smartInfoId).getCurrentPower();
		float available = powerThresholds.getFirstThreshold() - super.getTotalIstantaneousDemandPower()
				+ super.getIstantaneousProducedPower();
		return available;
	}
	...
	public EnergyPhaseScheduleTime[] getEnergyPhasesSchedule(String applianceId, short powerProfileID) {
		...
		if (ppi == null)
			throw new IllegalStateException("Unknown Profile ID " + powerProfileID);
	...
	public EnergyPhaseScheduleTime[] calculateEnergyPhasesSchedule(String applianceId, short powerProfileID) {
		...
		/*
		 * switch (profileState.getState()) { case
		 * PowerProfileState.POWER_PROFILE_PROGRAMMED: // not running, so the
		 * whole PP can be scheduled break;
		 * 
		 * case PowerProfileState.ENERGY_PHASE_WAITING_TO_START: // for the time
		 * being, only if the 1st phase is considered then the scheduling is
		 * possible if (profileState.getEnergyPhaseID() == 1) break;
		 * 
		 * // all these cases are not dealt with by now case
		 * PowerProfileState.POWER_PROFILE_IDLE: case
		 * PowerProfileState.ENERGY_PHASE_PAUSED: case
		 * PowerProfileState.ENERGY_PHASE_RUNNING: case
		 * PowerProfileState.ENERGY_PHASE_WAITING_PAUSED: default: throw new
		 * IllegalStateException("Unschedulable Profile State " +
		 * PowerProfileState.getNameOf(profileState)); }
		 */
	...
	public EnergyPhaseScheduleTime[] runSchedule(PowerProfileInfo ppi) {
		try {
			...
			if (smartInfoProduction != null) {
				...
				List<Float> hourlyData = getCloudServiceProxy().retrieveHourlyProducedEnergyForecast(
						smartInfoProduction.getApplianceId());
				if (hourlyData != null && hourlyData.size() > SolarIrradianceProfile.MINIMUM_INTERPOLATION_HOURS) {
					// set current value as most accurate than any forecast
					// (obvious)
					...
					if (sky == null)
						sky = new SolarIrradianceProfile(maxProducedPower, EnergyAllocator.NUMBER_OF_DAYS_HORIZON,
								SkyCover.ClearSky);
	...
	private PowerProfileInfo getOrRetrievePowerProfile(WhiteGoodInfo appliance, short profileId) {
		...
		if (ppi == null)
			ppi = powerControlProxy.retrievePowerProfile(appliance.getApplianceId(), profileId);
		if (ppi != null)
			appliance.notifyPowerProfile(ppi);
		return ppi;
	}
