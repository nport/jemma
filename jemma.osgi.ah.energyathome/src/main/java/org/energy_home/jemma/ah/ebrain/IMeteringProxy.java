...
public interface IMeteringProxy {
	// public static final short FORMATTING_DECIMAL_DIGITS_MASK = 0x7; // bits
	// 0-2
	// public static final short FORMATTING_INTEGER_DIGITS_MASK = 0x38; // bits
	// 3-6
	...
	// enumerated values representing Energy, Gas, Water, Thermal, and mirrored
	// metering devices
	short getMeteringDeviceType(String applianceId);

	// provides a method to properly decipher the number of digits and the
	// decimal
	// location of the values found in the Summation Information Set of
	// attributes
	float getSummationFormatting(String applianceId);

	// represents the most recent summed value of Energy and consumed in the
	// premise.
	double getCurrentSummationDelivered(String applianceId);

	void subscribeCurrentSummationDelivered(String applianceId, long minReportingInterval, long maxReportingInterval,
			double deltaValue);
	...
	void subscribeCurrentSummationReceived(String applianceId, long minReportingInterval, long maxReportingInterval,
			double deltaValue);
	...
	// represents the current Demand of Energy delivered or received at the
	// premise.
	// Positive values indicate Demand delivered to the premise where negative
	// values
	// indicate demand received from the premise. In E@H describes the total
	// power used
