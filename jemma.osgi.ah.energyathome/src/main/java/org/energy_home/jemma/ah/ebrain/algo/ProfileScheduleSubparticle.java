...
final class ProfileScheduleSubparticle {
	...
	/*
	 * public static final double nextRandom(int randomDistribution) { double r;
	 * switch (randomDistribution) { case GAUSSIAN_STEP: return
	 * GAUSSIAN_CONSTRICTION * rand.nextGaussian();
	 * 
	 * case ABS_GAUSSIAN_STEP: return GAUSSIAN_CONSTRICTION *
	 * Math.abs(rand.nextGaussian());
	 * 
	 * case CAUCHY_STEP: return CAUCHY_CONSTRICTION * Math.tan(Math.PI *
	 * (rand.nextDouble() - 0.5));
	 * 
	 * case ABS_CAUCHY_STEP: return CAUCHY_CONSTRICTION * Math.tan(HALF_PI *
	 * rand.nextDouble());
	 * 
	 * case EXPONENTIAL_STEP: r = Math.log(rand.nextDouble()); return
	 * EXPONENTIAL_CONSTRICTION * (rand.nextDouble() < 0.5 ? -r : r);
	 * 
	 * case ABS_EXPONENTIAL_STEP: return EXPONENTIAL_CONSTRICTION *
	 * -Math.log(rand.nextDouble());
	 * 
	 * case LEVY_STEP: r = nextLevy(); return LEVY_CONSTRICTION *
	 * (rand.nextDouble() < 0.5 ? -r : r);
	 * 
	 * case ABS_LEVY_STEP: return LEVY_CONSTRICTION * nextLevy();
	 * 
	 * default: return rand.nextDouble(); } }
	 */
	...
	// tardiness is the difference between the current profile end and the
	// minimum theoretical termination
	float getTardiness() {
		return tardiness;
	}
	...
	void nextRandomFlight(ProfileScheduleSubparticle bestParticle) {
		// Set globalBestPositions Array to the best Particle's best Positions
		// Array
		...
		for (int i = 0; i < phases.length; i++) {
			// If i is greater than 0 (i.e. for all Pahses other than the firts
			// one)
			if (i > 0) {
				// Set phaseMaxDelay to the minimum between currentMaxSlack, and
				// Phase(i)'s MaxAlowedDelay
			...
			// Initialise attractor to r multiplied by Phase(i)'s
			// currentBestPositions(i) plus ( 1 minus r ) multiplied by
			// Phases(i)'s globalBestPositions(i)
			...
			double c = nextCauchy();
			// Initialise step to c multiplied by (attractor minus Phase(i)'s
			// currentPositions(i))
			...
			// If Phase(i)'s currentPosition is less than 0 or greater than
			// phaseMaxDelay
			if (phasesCurrentPositions[i] < 0 || phasesCurrentPositions[i] > phaseMaxDelay)
				// Set Phase(i)'s currentPosition to 0
				phasesCurrentPositions[i] = 0;

			// Subtract to currentMaxSlack the new updated Phase(i)'s
			// currentPosition
	...
	void setEnergyPhasesBestSchedule() {
		// 1st phase schedule is set as absolute, while all other phases are
		// relative delays
