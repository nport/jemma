...
public abstract class DailyTariff {
	...
	// private static List<DailyTariff> singletons = new
	// ArrayList<DailyTariff>();
	/*
	 * static Reference<DailyTariff> getWeakInstance() { return new
	 * WeakReference<DailyTariff>(instance); }
	 */
	public static DailyTariff getInstance() throws InstantiationException, IllegalAccessException {
		if (instance == null)
			return getInstance(TwoTierDailyTariff.class);
		return instance;
	}
	...
	/*
	 * static <T extends DailyTariff> T getInstance2(Class<T> clazz) throws
	 * InstantiationException, IllegalAccessException { for (DailyTariff
	 * instance : singletons) { if (instance.getClass() == clazz) return
	 * clazz.cast(instance); } log.debug("new tariff " + clazz.getSimpleName());
	 * T instance = clazz.newInstance(); singletons.add(instance); return
	 * instance; }
	 */
	// this matrix is a 7 rows, each for every day of the week, and every column
	// are the
	...
	protected void setDailyTariff(TariffIntervals[] tariffIntervals, int dayOfWeek) throws Exception {
		...
		// 1st check to see if we already have a 'normalized' slot daily-tariff,
		// else make a new one;
		for (int i = 0; i < 7; ++i) {
		...
		for (TariffIntervals ti : tariffIntervals) {
			// the tariff is expressed 'per hour', so divide by the number of
			// slots in one hour
	...
	private float[] getSlotDailyTariffProfile(Calendar calendar) {
		...
		if (dayOfWeek != Calendar.SUNDAY && CalendarUtil.isHolyday(calendar))
			dayOfWeek = Calendar.SUNDAY;
		return slotsWeekTariffs[dayOfWeek - 1];
	}

	// compute the cost of a 'flat' energy value in the specified duration
	// interval
	...
	public EnergyCostInfo computeMinMaxCosts(Calendar calendar, long duration, double deltaEnergy) {
		...
		// NOTE: the tariff is Kwatt/hour, while the 'energy' parameter is in
		// watt/hour
	...
	// compute the cost using the energy allocated in the buffer. The boolean
	// flag specifies
	// whether the buffer is time relative to the Calendar object or absolute,
	// i.e. if the
	 ...
	 * return computeCost1(calendar, energyAllocation, true); } public float
	 * computeCost1(Calendar calendar, float[] energyAllocation, boolean
	 * absoluteTimeAllocation) { int dayOfWeek =
	 * calendar.get(Calendar.DAY_OF_WEEK) -1; int tariffSlot =
	 * CalendarUtil.getSlotOf(calendar); int startSlot = absoluteTimeAllocation
	 * ? tariffSlot : 0; int endSlot = energyAllocation.length; while
	 * (energyAllocation[--endSlot] == 0);
	 * 
	 * TariffIntervals[] tariffIntervals = weekTariffIntervals[dayOfWeek];
	 * 
	 * double cost = 0; while (startSlot <= endSlot) { // find the tariff
	 * interval that includes this time slot for (int i = 0; i <
	 * tariffIntervals.length; ++i) { int[] slotBounds =
	 * tariffIntervals[i].getSlotIntervals(); float tariff =
	 * tariffIntervals[i].getTariff(); for (int t = 0; t < slotBounds.length; t
	 * += 2) { while (tariffSlot >= slotBounds[t] && tariffSlot <
	 * slotBounds[t+1]) { // interval found, now get the intersection cost +=
	 * tariff * energyAllocation[startSlot++]; if (++tariffSlot >=
	 * CalendarUtil.DAILY_TIME_SLOTS) { // roll to the next day tariffIntervals
	 * = weekTariffIntervals[++dayOfWeek %7]; tariffSlot -=
	 * CalendarUtil.DAILY_TIME_SLOTS; } } } } } // the tariff is expressed 'per
	 * hour', so divide by the number of slots in one hour return (float)cost;//
	 * * CalendarUtil.SLOT_TARIFF_COEFFICIENT; }
	 */
	...
	public static class TariffIntervals {
		...
		public TariffIntervals(float tariff, int[] intervals) {
			...
			if ((intervals.length % 2) != 0)
				throw new IllegalArgumentException("Uneven time intervals.");
			this.tariff = tariff;
		...
		public boolean equals(Object o) {
			...
			if (tariff != other.getTariff())
				return false;
			int[] otherIntervals = other.getTimeIntervals();
			if (timeIntervals.length != otherIntervals.length)
				return false;
			for (int i = 0; i < timeIntervals.length; ++i)
				if (timeIntervals[i] != otherIntervals[i])
					return false;
			return true;
		}
