...
public class ApplianceInfo {
	...
	public static long MAX_VALID_EPOC_THRESHOLD = 100 * 24 * 3600 * 1000L; // 100
																			// days
																			// *
																			// 24
																			// hours
	...
	public static long MAX_SCHEDULING_WINDOW = 24 * 60 * MILLISECS_IN_ONE_MINUTE - 100; // 24
																						// hours

	// public static short priceDecimals = TRAILING_DIGIT_TENTHS;

	// private static final PowerProfileInfo[] EMPTY_PROFILES = new
	// PowerProfileInfo[0];
	 ...
	 */
	// private float energyFormatting =
	// IPowerProfileProxy.INVALID_FORMATTING_VALUE;
	/*
	...
	private double lastTotalEnergy; // last value of the energy sent by the
									// appliance
	private long lastNotificationTime;
	private long lastValidEnergyTime;

	private MinMaxPowerInfo powerInfo = new MinMaxPowerInfo();
	private EnergyCostInfo lastCostEnergy = new EnergyCostInfo(); // last value
																	// of the
																	// energy in
																	// the cost
																	// calculation
	...
	/*
	 * public ApplianceType getApplianceType() { return type; } public void
	 * setApplianceType(ApplianceType type) { this.type = type; }
	 */
	...
	/*
	 * public short getTotalProfileNum() { return totalProfileNum; } public void
	 * setTotalProfileNum(short totalProfileNum) { this.totalProfileNum =
	 * totalProfileNum; ensureCapacityProfiles(totalProfileNum); }
	 * 
	 * public boolean isMultipleSchedulingSupported() { return
	 * multipleSchedulingSupported; } public void
	 * setMultipleSchedulingSupported(boolean multipleSchedulingSupported) {
	 * this.multipleSchedulingSupported = multipleSchedulingSupported; }
	 * 
	 * public float getEnergyFormatting() { return energyFormatting; }
	 * 
	 ...
	 */
	/*
	 * private void ensureCapacityProfiles(short totalNum) { if
	 * (powerProfileInfos.length < totalNum) { PowerProfileInfo[] ppi = new
	 * PowerProfileInfo[totalNum +1]; System.arraycopy(powerProfileInfos, 0,
	 * ppi, 0, powerProfileInfos.length); powerProfileInfos = ppi; } }
	 * 
	 * public PowerProfileInfo getPowerProfile(short profileId) { if (profileId
	 * >= powerProfileInfos.length) return null; return
	 * powerProfileInfos[profileId]; }
	 * 
	 * public void notifyPowerProfile(PowerProfileInfo ppi) {
	 * setTotalProfileNum((short)Math.max(ppi.getTotalProfileNum(),
	 * ppi.getProfileId()));
	 * 
	 * PowerProfileInfo current = getPowerProfile(ppi.getProfileId()); if
	 * (current != null) {
	 * ppi.setProfileCurrentState(current.getProfileCurrentState());
	 * ppi.setTimeConstraints(current.getTimeConstraints());
	 * //ppi.setProposedScheduledEnergyPhases
	 * (current.getProposedScheduledEnergyPhases());
	 * //ppi.setActualScheduledEnergyPhases
	 * (current.getActualScheduledEnergyPhases()); }
	...
	public EnergyCostInfo updateEnergyCost(long newTime, double newEnergy) {
		...
		if (newTime < lastNotificationTime) {
			// FIXME by Riccardo: do we have any special reason for all these
			// String.format calls inside debug messages in this
			// class ?? if not, we should use normal logging messages to avoid
			// function calls - leaving as it is right now
			// LOG.error(String.format("updateEnergyCost %s: received new time value %d before last valid time value %d, returning null",
			// applianceId, newTime, lastNotificationTime));
			LOG.warn("updateEnergyCost " + applianceId + ": received new time value " + newTime + " before last valid time value "
					+ lastNotificationTime + ", returning null");
			return eci;
		}
		...
		try {
			...
			if (elapsedTime > 0 && elapsedTime < MAX_VALID_EPOC_THRESHOLD) {
				...
				LOG.warn(String
						.format("updateEnergyCost %s: received new time value %d , elapsed time %d > MAX_VALID_EPOC_THRESHOLD, resetting start time",
								applianceId, newTime, elapsedTime));
	...
	public void setIstantaneousPower(float ic, long time) {
		if (ic == IMeteringProxy.INVALID_INSTANTANEOUS_POWER_VALUE)
			ic = 0;
