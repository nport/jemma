...
public class PowerProfilePhaseExtended {
	// A % number that bias the allocated power consumption between the peak
	// power (100%)
	...
	// the average power is exactly the mean of the energy in the phase's
	// duration
	private float averagePower;

	// the allocated power consumption during the scheduling which is a biased
	// compromise
	// between the average and the peak power (see below)
	private float biasedPower;

	// the scheduled delay of this phase w.r.t. the previous one. For the 1st
	// phase this
	// corresponds to the delay of the power-profile w.r.t. to the "current"
	// time.
	...
	public PowerProfilePhaseExtended(PowerProfileTransferredPhase phase, float energyConverter) {
		...
		if (phase.ExpectedDuration == INDEFINITE_DURATION || phase.Energy == 0) {
			...
			// now the energy is in W/h while duration is in minutes so multiply
			// by 60 and divide by duration.
		...
		if (phase.ExpectedDuration < INDEFINITE_DURATION && phase.ExpectedDuration > 0)
