...
public class EnergyBrainOld {
	...
	private static long encodeCostWithFormatting(float value, Short formatting) {
		...
		if (decimals < 0)
			throw new IllegalArgumentException("Incosisten formatting.");
	...
	static {
		instance = new EnergyBrainOld();
	}

	public static EnergyBrainOld getInstance() {
		return instance;
	}
	...
	// private Map<String, AccumulatedEnergyCostPowerInfo> costs = new
	// HashMap<String, AccumulatedEnergyCostPowerInfo>();
	...
	/*
	 * public synchronized Map<String, AccumulatedEnergyCostPowerInfo>
	 * getAccumulatedCostInfos() { return costs; }
	 * 
	 * public synchronized void setAccumulatedCostInfos(Map<String,
	 * AccumulatedEnergyCostPowerInfo> map) { costs = map; for (Entry<String,
	 * AccumulatedEnergyCostPowerInfo> entry : map.entrySet()) { SmartAppliance
	 * appliance = getAppliance(entry.getKey());
	 * appliance.setAccumulatedEnergyCost(entry.getValue()); } }
	 * 
	 * /* //TODO generate appliance from restored state public void
	 * setApplianceDescriptor(ApplianceDescriptor ad, String applianceId) { try
	 * { SmartAppliance appliance = getAppliance(applianceId, true); } catch
	 * (ApplianceException e) {} }
	 */
	public void notifyApplianceState(short state, String applianceId) {
		getAppliance(applianceId).setState(state);
	}

	public void notifyPowerProfile(short totalProfileNum, short powerProfileID, PowerProfileTransferredPhase[] phases,
			String applianceId) {
	...
	public PowerProfilePrice getPowerProfilePrice(short powerProfileID, int delay, String applianceId)
			throws IllegalArgumentException {
		if (!appliances.containsKey(applianceId))
			throw new IllegalArgumentException(applianceId + " Appliance Not Found.");
		...
		if (delay > 0)
			scheduledTime.add(Calendar.MINUTE, delay);
		...
		for (int i = 0; i < phases.length; ++i) {
			if (phases[i].PeakPower > maxPeakPower)
				maxPeakPower = phases[i].PeakPower;
			// TODO check correct position of the decimal in energy to cast from
			// long to double
	...
	public ScheduledPhase[] proposeProfileSchedule(ApplianceInfoOld app, short profileId) {
		...
		if (profileState != null) {
			if (!profileState.PowerProfileRemoteControl)
				return null;
			...
			case ApplianceInfoOld.ENERGY_PHASE_RUNNING:
				// this very phase cannot be rescheduled. We need to know when
				// this phase
				// completes and use that as the earliest start for the next
				// phase and also use the
			...
			case ApplianceInfoOld.ENERGY_PHASE_WAITING_TO_START:
				// this very phase can be rescheduled. We need to know when its
				// max delay will
		...
		/*
		 * ParticleSwarmScheduler swarm = new ParticleSwarmScheduler(appliance,
		 * new EnergyAllocator(c, dt), swarmSize); int iterations =
		 * swarm.run(timeLimit); swarm.setBestSchedule(); ScheduledPhase[]
		 * proposal = appliance.generateSchedule();
		 * appInfo.setProposedScheduledPhases(proposal);
		 */
		return null;
	}

	// propose a reschedule of the current scheduled devices to see if there's
	// improvement.
	// Perhaps use a different energy allocator.
	public Map<String, ScheduledPhase[]> proposeOverallSchedule() {
		// ????? which scheduled appliances should be considered, what criteria
		// make sense?
		// defined a structure to return the overall reschedule
		return null;
	}

	// called to notify of the scheduled appliance. Must adjust energy allocator
	// with the info.
	...
	public MinMaxPowerInfo getTotalInstantaneosPowerInfo(String applianceId) {
		...
		for (SmartAppliance sa : appliances.values()) {
			if (sa.getApplianceId().equals(applianceId))
				smartInfoPower = sa.getMinMaxPowerInfo();
			else
				summedPower += sa.getIstantaneousPower();
		}
		// this case the SmartInfo has not been found yet
		if (smartInfoPower == null)
			smartInfoPower = new MinMaxPowerInfo();
	...
	/*
	 * public float getApplianceInstantaneosPowerUsage(String applianceId) { if
	 * (!appliances.containsKey(applianceId)) throw new
	 * IllegalArgumentException(applianceId + " Appliance Not Found.");
	 * 
	 * SmartAppliance appliance = getAppliance(applianceId); return
	 * appliance.getIstantaneousPower(); }
	 * 
	 * public long getApplianceInstantaneosPowerTime(String applianceId) { if
	 * (!appliances.containsKey(applianceId)) throw new
	 * IllegalArgumentException(applianceId + " Appliance Not Found.");
	 * 
	 * SmartAppliance appliance = getAppliance(applianceId); return
	...
	public SmartAppliance getAppliance(String applianceId) {
		/*
		 * if (detector == null) setEnergyBrainListener(new
		 * EnergyBrainListener() {
		 * 
		 * 
		 * public void notifyUnderload(SmartAppliance a) { // TODO
		 * Auto-generated method stub
		 * 
		 * }
		 * 
		 * 
		 * public void notifySafeLoad() { // TODO Auto-generated method stub
		 * 
		 * }
		 * 
		 * 
		 * public void notifyOverload(SmartAppliance a) { // TODO Auto-generated
		 * method stub
		 * 
		 * } });
		 */
		...
		if (appliance == null) {
			...
			if (detector != null)
				detector.addAppliance(appliance);
		}
		return appliance;
	}
