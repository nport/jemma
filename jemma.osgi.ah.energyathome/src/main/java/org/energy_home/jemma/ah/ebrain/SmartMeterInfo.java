...
public class SmartMeterInfo extends ApplianceInfo {
	...
	// public static final int PRODUCED_ENERGY_MIN_INTERVAL =
	// 10000;//MILLISECONDS_IN_MINUTE;
	...
	// Used to limit the number of received energy values that are used (-1
	// means all values are used)
	private int nextTotalEnergyValidValues = -1;
	private int nextProducedEnergyValidValues = -1;

	private double lastProducedEnergy; // last value of the energy sent by the
										// appliance
	...
	public EnergyCostInfo updateProducedEnergy(long newTime, double newEnergy) {
		...
		if (newTime < lastValidProducedEnergyTime) {
			LOG.warn(String.format(
					"updateProducedEnergy: received new time value %d before last valid time value %d, returning null", newTime,
					lastValidProducedEnergyTime));
			return eci;
		}
		...
		if (elapsedTime > 0 && elapsedTime < MAX_VALID_EPOC_THRESHOLD) {
			...
			LOG.warn(String
					.format("updateProducedEnergy: received new time value %d , elapsed time %d > MAX_VALID_EPOC_THRESHOLD, resetting start time",
							newTime, elapsedTime));
