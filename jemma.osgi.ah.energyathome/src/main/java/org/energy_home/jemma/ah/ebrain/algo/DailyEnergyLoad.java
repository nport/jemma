...
public class DailyEnergyLoad {
	...
	// The best results were obtained with the smoothing constants alpha=0.7
	// beta=0 (no trend) and gamma=0.3
	...
	public void update(int currentSlot) {
		if (updates == DAILY_SLOTS)
			computeSeasonalSmoothing();

		int previousSlot = currentSlot == 0 ? DAILY_SLOTS - 1 : currentSlot - 1;
		if (updates == 0)
			smoothedLevel[currentSlot] = realData[currentSlot];
		else if (updates < DAILY_SLOTS) {
			smoothedLevel[currentSlot] = alpha * realData[currentSlot] + (1 - alpha) * smoothedLevel[previousSlot];
		} else {
			smoothedLevel[currentSlot] = alpha * realData[currentSlot] / smoothedSeason[currentSlot] + (1 - alpha)
					* smoothedLevel[previousSlot];
			smoothedSeason[currentSlot] = gamma * realData[currentSlot] / smoothedLevel[currentSlot] + (1 - gamma)
					* smoothedSeason[currentSlot];
		}
		++updates;
	}
