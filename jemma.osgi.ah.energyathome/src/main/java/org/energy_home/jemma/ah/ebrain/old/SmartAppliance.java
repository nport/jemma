...
public class SmartAppliance {
	...
	public static String stateToString(short state) {
		...
		case STATE_PENDING_SUSPENSION:
			return "STATE_PENDING_SUSPENSION";
		case STATE_ACTIVE:
			return "STATE_ACTIVE";
		case STATE_PENDING_ACTIVATION:
			return "STATE_PENDING_ACTIVATION";
		default:
	...
	public static long MAX_SCHEDULING_WINDOW = 18 * 60 * MILLISECS_IN_ONE_MINUTE - 100; // 24
																						// hours
	public static float INVALID_POWER_VALUE = 0xFFFFFF;
	public static double INVALID_ENERGY_VALUE = 0xFFFFFFFFFFFFL;
	// public static double MAX_VALID_ENERGY_THRESHOLD = 21000D / (3600 * 1000);
	// // max delta energy per millisec (21KW/H)
	...
	public float allocatePowerProfile(float[] energyAllocation, int startSlot) {
		...
		for (int i = 0; i < extendedPhases.length; ++i) {
			if (extendedPhases[i].getRequiredPower() > maxPeakPower)
				maxPeakPower = extendedPhases[i].getRequiredPower();
	...
	public long getLatestProfileEnd() {
		if (endBefore - startAfter <= 0)
			endBefore = startAfter + MAX_SCHEDULING_WINDOW;
		return endBefore;
	}
