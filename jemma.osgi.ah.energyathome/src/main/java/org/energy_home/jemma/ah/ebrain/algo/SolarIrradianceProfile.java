...
public class SolarIrradianceProfile {
	...
	public static float[] interpolate(List<Float> hourlyData) {
		if (hourlyData == null)
			throw new IllegalArgumentException("The hourly data cannot be null.");
		if (hourlyData.size() < MINIMUM_INTERPOLATION_HOURS)
			throw new IllegalArgumentException("The hourly data must contain at least " + MINIMUM_INTERPOLATION_HOURS);
		...
		while (finalIndex > MINIMUM_INTERPOLATION_HOURS && hourlyData.get(finalIndex) == null)
			--finalIndex;
		while (finalIndex > MINIMUM_INTERPOLATION_HOURS && hourlyData.get(finalIndex) == 0)
			--finalIndex;

		if (hourlyData.get(finalIndex) == null)
			hourlyData.set(finalIndex, 0.0f);

		// normalize to minute
		for (int i = finalIndex; i >= 0; --i) {
			Float value = hourlyData.get(i);
			if (value != null && value > 0)
				hourlyData.set(i, value * CalendarUtil.HOURS_IN_ONE_SLOT);
		}
	...
	public SolarIrradianceProfile(float maxPower, int days, SkyCover sky) {
		...
		initializeClearSky();
		if (sky == SkyCover.Overcast)
			initializeOvercast();
		if (sky == SkyCover.PartlyClouded)
			initializePartlyClouded();
	}
	...
	private void initializeOvercast() {
		for (int i = series.length; --i >= 0;) {
			if (series[i] <= 0)
				...
				series[i] = (float) Math.pow(series[i], 0.6);
		}
	}
	...
	public String toString() {
		...
		for (int i = 0; i < CalendarUtil.SLOTS_IN_ONE_DAY; ++i) {
			...
			if ((i % CalendarUtil.SLOTS_IN_ONE_HOUR) == 0)
				sb.append('\n');
		}
		return sb.toString();
	}
