...
public class ESPHapServiceObject extends ESPContainersDataUtils implements ICloudServiceProxy {
	...
	public AHContainerAddress getReceivedEnergyApplianceContainerId(int resolution, String applianceId) throws ESPException {
		...
		try {
			...
			switch (resolution) {
			case ESPService.HOUR_RESOLUTION:
				result = hapService.getHagContainerAddress(deviceIds[0], deviceIds[1],
	...
	public AHContainerAddress getEnergyCostApplianceContainerId(int resolution, String applianceId) throws ESPException {
		...
		try {
			...
			case ESPService.DAY_RESOLUTION:
				result = hapService.getHagContainerAddress(deviceIds[0], deviceIds[1],
						EHContainers.attrId_ah_eh_esp_dailyEnergyCost);
				break;
			case ESPService.MONTH_RESOLUTION:
				result = hapService.getHagContainerAddress(deviceIds[0], deviceIds[1],
		...
		if (items != null) {
			...
			if (resultList != null) {
				...
				LOG.info("getFloatValueMonthlyForecast - current month daily consumption returned: monthTotal="
						+ currentMonthEstimation + ", monthTotalDuration=" + totalDuration + ", lastValue=" + lastValue
						+ ", lastTime=" + lastTime + ", lastDuration=" + lastDuration);
			}
		} else {
			return null;
		}
	...
	public ContentInstanceItems getHourlyProducedEnergyForecast(AHContainerAddress containerId) throws M2MHapException {
		...
		if (result == null) {
			...
			long startTime = containerName.equals(EHContainers.attrId_ah_eh_esp_hourlyReceivedEnergy) ? System.currentTimeMillis()
					- ONE_DAY_IN_MILLISEC : System.currentTimeMillis();
			long startInstanceId = getNormalizedStartTime(c, startTime, ESPService.HOUR_RESOLUTION);
			long endInstanceId = getNormalizedEndTime(c, startTime + ONE_DAY_IN_MILLISEC * 3 - ONE_HOUR_IN_MILLISEC,
					ESPService.HOUR_RESOLUTION);
	...
	public List<Float> retrieveHourlyProducedEnergyForecast(String applianceId) {
		...
		try {
			String[] deviceIds = ESPApplication.getDeviceIds(applianceId);
			AHContainerAddress containerId = hapService.getHagContainerAddress(deviceIds[0], deviceIds[1],
		...
		// produced energy data collected in the previous 24
		// hours)
		try {
	...
	public void storeDeliveredEnergyCostPowerInfo(String applianceId, EnergyCostInfo eci, MinMaxPowerInfo pi)
			throws M2MHapException {
		if (espGwTodayCache != null) {
		...
		if (cost != null) {
			...
			// FIXME: NICOLA: removed this comment because now the items doesn't
			// have static methods anymore!
	...
	public ContentInstanceItems getItems(Calendar c, AHContainerAddress containerId, long startInstanceId, long endInstanceId)
			throws M2MHapException {
		LOG.info("getItems(containerId=" + containerId + ", startInstanceId=" + startInstanceId + ", endInstanceId="
				+ endInstanceId + ")");
		...
		if (espGwTodayCache != null) {
			...
			if (cacheResult != null && cacheResult.isComplete()) {
				LOG.info("getItems resolved on local gw cache");
			...
			if (cacheResult != null) {
				...
				LOG.info("getItems result merged with local gw cache");
			}
		} else {
			result = hapService.getContentInstanceItems(containerId, startInstanceId, endInstanceId);
		}
		if (result != null && result.getContentInstances() != null)
			LOG.info("getItems returned: contentInstances size=" + result.getContentInstances().size());
		else if (result == null)
			LOG.error("getItems returned: contentInstanceItems=null");
		else
			LOG.error("getItems returned: contentInstances=null");
		return result;
	}

	public ContentInstanceItems getItemsWithHapCache(Calendar c, AHContainerAddress containerId, long startInstanceId,
			long endInstanceId, int resolution) throws M2MHapException {
	...
	public ContentInstanceItems getNormalizedItems(Calendar c, AHContainerAddress containerId, long startInstanceId,
			long endInstanceId, int resolution) throws M2MHapException {
	...
	public List<Float> getReceivedEnergy(String applianceId, long startTime, long endTime, int resolution) throws M2MHapException,
			ESPException {
	...
	public List<Float> getEnergyCost(String applianceId, long startTime, long endTime, int resolution) throws M2MHapException,
			ESPException {
	...
	public ContentInstanceItemsList getItemsList(Calendar c, AHContainerAddress containerIdFilter, long startInstanceId,
		...
		ContentInstanceItemsList itemsList = hapService.getContentInstanceItemsList(containerIdFilter, startInstanceId,
				endInstanceId);
		if (itemsList != null && itemsList.getContentInstanceItems() != null)
			LOG.info("getItemsList returned: contentInstances size=" + itemsList.getContentInstanceItems().size());
	...
	public ContentInstanceItemsList getItemsListWithHapCache(Calendar c, AHContainerAddress containerId, long startInstanceId,
			long endInstanceId, int resolution) throws M2MHapException {
		// TODO: local cache for query on hap server are currently used only for
		// week day hourly average values
	...
	public ContentInstanceItemsList getNormalizedItemsList(Calendar c, AHContainerAddress containerId, long startInstanceId,
			long endInstanceId, int resolution) throws M2MHapException {
	...
	public Map<String, List<Float>> getEnergyConsumption(long startTime, long endTime, int resolution) throws M2MHapException,
			ESPException {
	...
	public Map<String, List<Float>> getEnergyCost(long startTime, long endTime, int resolution) throws M2MHapException,
			ESPException {
	...
	public ContentInstanceItems getWeekDayItems(AHContainerAddress containerId, long startInstanceId, long endInstanceId)
			throws M2MHapException {
		ContentInstanceItems result = hapService.getContentInstanceItems(containerId, startInstanceId, endInstanceId);
		if (result != null && result.getContentInstances() != null)
			LOG.info("getWeekDayItems returned: contentInstances size=" + result.getContentInstances().size());
		else if (result == null)
	...
	public ContentInstanceItems getWeekDayItemsWithHapCache(AHContainerAddress containerId, long startInstanceId, long endInstanceId)
			throws M2MHapException {
		ContentInstanceItems result = null;
		if (espHapCache != null)
	...
	public ContentInstanceItems getNormalizedWeekDayItems(AHContainerAddress containerId, long startInstanceId, long endInstanceId)
			throws M2MHapException {
	...
	public Float getEnergyConsumptionForecast(String applianceId, int resolution) throws M2MHapException, ESPException {
		...
		String[] deviceIds = ESPApplication.getDeviceIds(applianceId);
		AHContainerAddress wdHourlyAvgContainerId = hapService.getHagContainerAddress(deviceIds[0], deviceIds[1],
				EHContainers.attrId_ah_eh_esp_wdHourlyEnergyAvg);
