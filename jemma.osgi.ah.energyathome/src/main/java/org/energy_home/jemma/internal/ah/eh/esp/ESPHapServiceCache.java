...
public class ESPHapServiceCache {
	...
	private Map<AHContainerAddress, QueryResult> cacheItemsMap = new HashMap<AHContainerAddress, QueryResult>(
			ESPApplication.MAX_NUMBER_OF_APPLIANCES);
	private Map<AHContainerAddress, QueryResult> cacheItemsListMap = new HashMap<AHContainerAddress, QueryResult>(
			ESPApplication.MAX_NUMBER_OF_APPLIANCES);
	private Map<AHContainerAddress, QueryResult> cacheWeekDayItemsMap = new HashMap<AHContainerAddress, QueryResult>(
			ESPApplication.MAX_NUMBER_OF_APPLIANCES);
	private Map<AHContainerAddress, QueryResult> cacheHourlyProducedEnergyForecastItemsMap = new HashMap<AHContainerAddress, QueryResult>(
			ESPApplication.MAX_NUMBER_OF_APPLIANCES);

	private static int getCurrentCacheResolution(Calendar calendar, AHContainerAddress containerId, long startTime, long endTime,
			int resolution) {
		int currentDay;
		int startDay;
		int endDay;
	...
	private static QueryResult getCachedQueryResult(Map<AHContainerAddress, QueryResult> cacheMap, Calendar calendar,
	...
	private static ContentInstanceItemsList filterContentInstanceItemsList(ContentInstanceItemsList itemsList, long startTime,
			long endTime) {
		if (itemsList == null)
			return null;
	...
	public ContentInstanceItems getCachedItems(Calendar calendar, AHContainerAddress containerId, long startTime, long endTime,
		...
		if (cache == null) {
			...
			if (items != null) {
				...
				// FIXME: NICOLA: removed this comment because now the items
				// doesn't have static methods anymore!
	...
	public ContentInstanceItemsList getCachedItemsList(Calendar calendar, AHContainerAddress containerId, long startTime,
			long endTime, int resolution) throws M2MHapException {
		int cacheResolution = getCurrentCacheResolution(calendar, containerId, startTime, endTime, resolution);
		if (cacheResolution == ESPService.NO_VALID_RESOLUTION)
			return null;
		...
		if (cache == null) {
			ContentInstanceItemsList itemsList = hapObject.getNormalizedItemsList(calendar, containerId, cacheStartTime,
					cacheEndTime, resolution);
			if (itemsList != null) {
	...
	public ContentInstanceItems getWeekDayCachedItems(AHContainerAddress containerId, long startId, long endId)
	...
	public ContentInstanceItems getHourlyProducedEnergyForecastCachedItems(AHContainerAddress containerId) throws M2MHapException {
		QueryResult<ContentInstanceItems> cache = getCachedQueryResult(cacheHourlyProducedEnergyForecastItemsMap,
				Calendar.getInstance(), containerId);
		if (cache == null) {
